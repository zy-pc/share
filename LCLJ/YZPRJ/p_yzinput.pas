      ...
      begin
        ...
        if ( ( pub_yydm = '0013' ) and ( qry_yz.FieldByName( 'xmdm' ).asstring = '01000001000357' )
          ...
          ( ( pub_yydm = '0013' ) and ( qry_yz.FieldByName( 'lb' ).asstring = '治疗' ) and
           ...
           and (Pos('型',qry_yz.FieldByName( 'xmmc' ).asstring)>0))  ) ) then
        begin
          //三台县医院胰岛素开备用不需要验证 ,治疗类开皮试和输血不验证
        end
        else
        begin
          //------------------项目名称与代码一致性校验-------------------
...
begin
   if Application.MessageBox('删除当前记录？', '提示', MB_OKCANCEL +
    MB_ICONINFORMATION)=mrOk then
     qry_yz.DeleteRecords(arAllChapters);
end;
...
var
  z_zyh: string;
begin
  if lcljms then HideSomething else self.WindowState:=wsMaximized; //临床路径模板
...
begin
  ...
  try
    if ( gi_passenabled = 1 ) and ( qry_yz.RecordCount > 0 ) then
      proc_save_pass( 1, qry_yz, qry_yf );
  except
  end;
  qry_yz.EnableControls;
  saveyz;
