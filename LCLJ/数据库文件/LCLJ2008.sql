 ...
 * Author :       微软用户
 *
 * Date Created : Wednesday, February 29, 2012 13:58:12
...
CREATE UNIQUE INDEX [临床路径目录_PK] ON LCLJ_LCLJML(LJID)
go
IF EXISTS (SELECT * FROM sys.indexes WHERE object_id=OBJECT_ID('LCLJ_LCLJML') AND name='临床路径目录_PK')
    PRINT '<<< CREATED INDEX LCLJ_LCLJML.临床路径目录_PK >>>'
ELSE
    PRINT '<<< FAILED CREATING INDEX LCLJ_LCLJML.临床路径目录_PK >>>'
go
