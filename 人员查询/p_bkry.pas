...
begin
  ...
  ss := 'and (a.dm='''+Trim(Edit1.Text)+''' or a.mc like '''+Trim(Edit1.Text)+'%'+''' or a.pym like '''+'%'+Trim(Edit1.Text)+'%'+''')'
  else
  ss :='';
 qry1.Close;
 qry1.SQL.Clear;
 qry1.SQL.Add('select a.dm,a.mc,a.pym,isnull(a.czje,0.00) czje,a.sybz,a.ysbz,a.cfbz,a.mzcfbz,RTRIM(b.mc) zwmc,RTRIM(c.mc) zcmc from sys_czy a,sys_zwdm b,sys_zcdm c ');
//  if Trim(pub_bqdm)='' then   //有病区的科室
  qry1.SQL.Add(' where a.ksdm='''+pub_ksdm+''' and a.txbz=0 and a.zwbm*=b.dm and a.zcdm*=c.dm ');
// else
//  qry1.SQL.Add(' where a.ksdm in (select bqzydm from sys_bqzyfl where bqdm='''+pub_bqdm+''') and a.txbz=0 and a.zwbm*=b.dm and a.zcdm*=c.dm ');
  if ss <> '' then
  qry1.SQL.Add(ss);
 qry1.Open;
 DBGridEh1.SetFocus;
