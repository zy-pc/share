...
implementation
uses DY_LoginFrm, DY_PublicUnit, DY_FilterUnit, DY_ResultFrm, DY_SetupFrm,p_dm;
{$R *.dfm}

procedure TMain_frm.SetSelectField(Value: string);
begin
  if Value <> '' then
...
procedure TMain_frm.SetCQuyStr(Value: string);
begin
...
procedure TMain_frm.SetEQuyStr(Value: string);
begin
...
procedure TMain_frm.SetSumField(Value: string);
begin
  if Value <> '' then
...
procedure TMain_frm.SetGroupField(Value: string);
begin
  if Value <> '' then
...
procedure TMain_frm.SetJSField(Value: string);
begin
  if Value <> '' then
...
procedure TMain_frm.SetZjField(Value: string);
begin
  if Value <> '' then
...
function TMain_frm.GetSQL(QuyStr: string): string;
var
...
function TMain_frm.DeleteQuyTab(QIndex: Integer): Boolean; //删除查询数据
var
  sSql: string;
begin
...
procedure TMain_frm.SaveQuyData(ChkShow: Boolean = True);
var
...
procedure TMain_frm.ShowCheck(ChkStr, FieldName: string);
var
  i, j: integer;
  FileStr: string;
begin
...
procedure TMain_frm.TreeShow(VarTree: TTreeView);
var
...
procedure TMain_frm.tvMainChange(Sender: TObject; Node: TTreeNode);
var
  Bywm, sSql: string;
  PNode: TTreeNode;
  ADOSetTmp: TADODataSet;
...
procedure TMain_frm.SetBtnStyle;
begin
...
function TMain_frm.GetReportData(ReportIndex: Integer): Boolean;
var
  sSql: string;
  ADOSetTmp: TADODataSet;
begin
...
procedure TMain_frm.actAddExecute(Sender: TObject);
begin
  AddQuyReport;
end;

procedure TMain_frm.actDelExecute(Sender: TObject);
var
  sCopation: string;
  Qindex: Integer;
  PNode: TTreeNode;
...
procedure TMain_frm.actExitExecute(Sender: TObject);
begin
  Close;
end;

procedure TMain_frm.actFilterExecute(Sender: TObject);
var
...
procedure TMain_frm.actResultExecute(Sender: TObject);
var
  sSql: string;
begin
  GetsField;
...
procedure TMain_frm.actSetupExecute(Sender: TObject);
var
  sSql: string;
begin
  SetupFiled;
...
procedure TMain_frm.AddQuyReport;
var
  TableName: string;
  ADOSetTmp: TADODataSet;
  sSql: string;
...
begin
  if Assigned(ADOConnectionMain) then
  begin
...
procedure TMain_frm.cxgrdbtblvwShowEditing(Sender: TcxCustomGridTableView;
  AItem: TcxCustomGridTableItem; var AAllow: Boolean);
var
