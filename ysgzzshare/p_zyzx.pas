...
uses
  ...
  Db, DBGrids, Mask, DBCGrids, ACTIVEX;
const
...
//同时，将该长度位于的级数放至数组 len_level 中
  ...
  main_dwdl.close;
  main_dwdl.commandtext := 'select dm,名称 from ' + tb_name + ' order by ' + field_bm;
  main_dwdl.open;
...
//先保存当前目录
  ...
  ChDir(sDirName);
  hFindFile := FindFirstFile(PChar(sDirName+'\*.html'), FindFileData);
...
begin
  ...
  begin
    DoRemoveDir(P);  // DeleteDir(p)之前必先 DoRemoveDir(P) zhangwancai
    DeleteDir(p);
  end;
