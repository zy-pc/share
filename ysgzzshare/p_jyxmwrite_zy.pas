...
var
  ...
  i: integer;
  _xseBz: boolean;
  yzqf :string;    //区分新生儿医嘱和普通医嘱     1为普通医嘱，2为新生儿医嘱
...
//------calmhawk----2011-06-16---添加，新生儿医嘱录入----
...
begin
  ...
  begin
    ...
    if yzlb = '其它' then
    begin
      frm_yzinput_xse.qry_yz.FieldByName( 'xmdm' ).AsString := '其它';
      ...
      frm_yzinput_xse.qry_xmyz.FieldByName( 'xmdm' ).AsString := '其它';
    ...
    begin
      ...
      frm_yzinput_xse.qry_yz.FieldByName( 'jcff' ).value := '新生儿';
  ...
  begin
    ...
    if yzlb = '其它' then
    begin
      frm_yzinput.qry_yz.FieldByName( 'xmdm' ).AsString := '其它';
      frm_yzinput.qry_yz.FieldByName( 'xmmc' ).AsString := xmmc;
      frm_yzinput.qry_yz.FieldByName( 'dw' ).AsString := '';
      if frm_yzinput.sfhzbr then   //会诊开单科室判断
      begin
        frm_yzinput.qry_yz.FieldByName( 'kdks' ).AsString := frm_yzinput.yz_kdks; //会诊原科室
      end else
      begin
        frm_yzinput.qry_yz.FieldByName( 'kdks' ).AsString := pub_ksdm;   //当前科室
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          frm_yzinput.qry_yz.FieldByName('yzsx').AsString := '临时';
        end;
      end;
      frm_yzinput.qry_xmyz.post;
    end;
...
{-------------------------------------------------------------------------------
  过程名:    Tfrm_jyxmwrite_zy.xmmxinyz
  作者:      yangshuai
  日期:      2015.12.10
  参数:      无
  返回值:    无
...
//
{-------------------------------------------------------------------------------
  过程名:    Tfrm_jyxmwrite_zy.BitBtn1Click
  作者:      yansghuai
  日期:      2016.01.15
  参数:      Sender: TObject
  返回值:    无
...
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            frm_yzinput_xse.sp_cx_yp_yz.Parameters.ParamValues['@brfylb'] := frm_yzinput_xse.sp_jbxx.fieldbyname('fylb').asstring ;//21为绿色通道，沙雅
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            frm_yzinput.sp_cx_yp_yz.Parameters.ParamValues['@brfylb'] := frm_yzinput.sp_jbxx.fieldbyname('fylb').asstring ;//21为绿色通道，沙雅
  ...
  if PageControl1.ActivePageIndex = 2 then   //医嘱项目排序
  begin
    if pub_yydm<>'0003' then
    begin
      //新生儿标志
