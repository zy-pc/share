...
type
  ...
  public
    ...
    fzbh: string;
    function HowManycharacter(Const s:String):Integer;//德阳肿瘤医院 判断检验项目中英文及数字个数
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Application.MessageBox('请输入要查询病人的完整卡号！', '信息提示', 48);
        e_tmh.SetFocus;
        Exit;
      end;
      tmh := trim(e_tmh.text);
  ...
  if Trim(tmh) = '' then
  begin
    Application.MessageBox('请先输入条码号', '提示', 16);
    e_tmh.SelectAll;
    e_tmh.SetFocus;
  end
  else
...
begin
  ...
  if zt = '1' then
  begin
    Application.MessageBox('不能选择相同项目一起打印！', '信息提示', 48);
    exit;
  end;
  if qry_brxx.Active and (qry_brxx.RecordCount > 0) then
  begin
    proc_checkyb; //检查样本
    proc_getfzbh(qry_brxx); //生成检验单号
    if sfqyydhsz = false then
      proc_printtmh() //打印条码号
    else
      proc_printtmh1();
    //proc_in_cyjl; //写入采样记录正式表
    qry_brxx.close;
  end;
  ReadInfo(False);
  //bt_can.Click;
...
begin
  ...
  sp_brxx.ExecProc;
  Qry_brxx.Close;
  if pub_yydm='0266' then //广元昭化人民医院特殊修改
  begin
     Qry_brxx.SQL.Text :=
  ...
  begin
    Application.MessageBox('暂无可检验的项目', '提示', 16);
...
begin
  ...
  begin
    ...
    begin
      if ado_query.FieldByName('sfjy').AsBoolean then //判断是否检验
      begin
        //判断样本不为空,并且分组号为空
      ...
      if fzbh = '' then
      begin
        Application.MessageBox('提取检验分组号失败，请重试！', '错误提示',
          MB_OK + MB_ICONSTOP);
        abort;
      end;
      ado_query.Next;
...
//          end;
            ...
            begin
              ...
              begin
                ...
                if Vxmmc1.Caption = '' then
                  ...
                  Vxmmc1.Caption := Vxmmc1.Caption + ',' + Vtemp;
                if n + 3 >= _xmmcStr.Count then //项目为4-5个时保证第一行可以打印出前三项
                  Continue;
              ...
              begin
                ...
                if Vxmmc2.Caption = '' then
                  ...
                  Vxmmc2.Caption := Vxmmc2.Caption + ',' + Vtemp;
                if n + 6 >= _xmmcStr.Count then //保证前两栏可以打印出项目前六项
                  Continue;
          ...
          else if pub_yydm = '0269' then  //台山医院
          begin
          ...
          brcd1.BarCode := mysz[ii];
          brcd1.Draw(QRImage3.Canvas, 4, 0, 35, 0);
          // Draw(QRImage3.Canvas, int 向左移动, int 向下移动, 条码纵向长度, 旋转角度)
      ...
      begin //三医院处理
        ...
        //          begin
        //            Application.MessageBox('下午6到早上6点不能打印非急查项目！', '错误提示',
        //            MB_OK + MB_ICONSTOP);
        //            abort;
        //          end;
...
begin
  ...
  if sczt <> '0' then
  begin
    Application.MessageBox(pchar('检验条码组号为' + fzbh +
      '的检验条码打印失败！'),
...
begin
  ...
  begin
    if qry_brxx.FieldByName('sfjy').AsBoolean then
      ...
      begin
        Application.MessageBox(pchar('还有项目未设置样本,请先设置...'),
          '信息提示', 48);
        cxGrid2.SetFocus;
        Abort;
      end;
...
{-------------------------------------------------------------------------------
  过程名:    Tfrm_jydtmdy.ChangePrinter
  作者:      yangshuai
  日期:      2017.09.19
  参数:      _index: Integer
  返回值:    无
...
begin
  //  if ACellViewInfo.Item.Caption='是否检验' then
  //  begin
  //      if qry_brxx.FieldByName( 'sfjy' ).asboolean then
  //        qry_brxx.FieldByName( 'sfjy' ).AsBoolean := false
  //      else
...
begin
  ...
  //广元昭化医院要求更改  wangwei  20170411
  DM_data.qry_pub.Close;
  DM_data.qry_pub.SQL.text := 'select jytmdywjf from sys_xt_kg ';
  DM_data.qry_pub.Open;

  //所以如果是0，就表示必须要计费才能打印，那么zysf_zyfydx就有数据；
  //如果是1，就表示你打印的时候，可能未计费，那么zysf_zyfydx可能没有数据，那就从zybl_zyyz取对应字段
  if not dM_data.qry_pub.FieldByName('jytmdywjf').AsBoolean then   //为0时
  begin
    Qry_brxx.SQL.Text :=
  ...
  begin
    if Bhint then
    Application.MessageBox('该病人暂无可检验的项目', '提示', 16);
    bt_Print.Enabled := False;
    e_tmh.SetFocus;
    e_tmh.SelectAll;
  end;
...
begin
  if trim(e_tmh.text) = '' then
  begin
    Application.MessageBox('请输入要查询病人的病床号！', '信息提示', 48);
    e_tmh.SetFocus;
    Exit;
  end;
  bch := trim(e_tmh.text);
  ...
  begin
    ...
    Qry_brxx.Parameters.ParamByName('zyh').Value := zyh;
    Qry_brxx.Open;
  end else if  (pub_yydm='0272')  then  //灌阳妇幼保健院
  begin
     Qry_brxx.SQL.Text :=
    'select a.*,b.kdrq,c.sgys from DZBL_CYJL_zy_ls a,zybl_zyyz b,sys_yzxm c,zysf_zydj d where a.zyh=b.zyh and ' +
    ' a.fyid=b.id and a.zyh=d.zyh and a.zyh=:zyh and a.jyxmdm=c.xmdm order by a.yb,a.dldm,c.sgys asc';
