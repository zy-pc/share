object frm_czyrbjkb: Tfrm_czyrbjkb
  Left = 189
  Top = 169
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = #38376#35786#25910#36153#25805#20316#21592#26085#25253
  ClientHeight = 474
  ClientWidth = 1114
  Color = clBtnFace
  Font.Charset = GB2312_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = #23435#20307
  Font.Style = []
  OldCreateOrder = False
  Position = poMainFormCenter
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 16
  object Bevel1: TBevel
    Left = 0
    Top = 0
    Width = 1114
    Height = 40
    Align = alTop
    Shape = bsFrame
    Style = bsRaised
    ExplicitWidth = 787
  end
  object Label4: TLabel
    Left = 5
    Top = 11
    Width = 80
    Height = 16
    Caption = #32467#24080#26085#26399#65306
  end
  object Label24: TLabel
    Left = 201
    Top = 11
    Width = 112
    Height = 16
    Caption = #36873#25321#32467#36134#21333#21495#65306
  end
  object BitBtn3: TcxButton
    Left = 777
    Top = 6
    Width = 97
    Height = 29
    Caption = #20132#27454#34920#25171#21360
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    OnClick = BitBtn3Click
  end
  object BitBtn4: TcxButton
    Left = 990
    Top = 6
    Width = 77
    Height = 29
    Caption = #36820#22238
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    TabOrder = 2
    OnClick = BitBtn4Click
    NumGlyphs = 2
  end
  object dt1: TcxDateEdit
    Left = 83
    Top = 7
    EditValue = 37005.7026529861d
    Properties.OnChange = dt1Change
    TabOrder = 3
    Width = 110
  end
  object GroupBox1: TGroupBox
    Left = 0
    Top = 40
    Width = 173
    Height = 434
    Align = alLeft
    Caption = #24635#25910#36153#39069
    TabOrder = 5
    object Label9: TLabel
      Left = 4
      Top = 31
      Width = 48
      Height = 16
      Caption = #24635#37329#39069
    end
    object Label10: TLabel
      Left = 4
      Top = 73
      Width = 48
      Height = 16
      Caption = #24635#20154#27425
    end
    object Label12: TLabel
      Left = 4
      Top = 116
      Width = 64
      Height = 16
      Caption = #36864#36153#37329#39069
    end
    object Label13: TLabel
      Left = 4
      Top = 158
      Width = 64
      Height = 16
      Caption = #36864#36153#20154#27425
    end
    object Label20: TLabel
      Left = 4
      Top = 200
      Width = 64
      Height = 16
      Caption = #20316#24223#37329#39069
    end
    object Label19: TLabel
      Left = 4
      Top = 240
      Width = 64
      Height = 16
      Caption = #20316#24223#20154#27425
    end
    object Label14: TLabel
      Left = 4
      Top = 282
      Width = 64
      Height = 16
      Caption = #24212#20132#37329#39069
    end
    object lbl1: TLabel
      Left = 4
      Top = 307
      Width = 96
      Height = 16
      Caption = #35786#26597#36153#24635#20154#27425
    end
    object lbl5: TLabel
      Left = 6
      Top = 335
      Width = 80
      Height = 16
      Caption = #29616#37329#35786#26597#36153
    end
    object DBEdit1: TDBEdit
      Left = 72
      Top = 28
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'sfje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
    end
    object DBEdit2: TDBEdit
      Left = 72
      Top = 70
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'sfrc'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 1
    end
    object DBEdit3: TDBEdit
      Left = 72
      Top = 111
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'tfje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 2
    end
    object DBEdit4: TDBEdit
      Left = 72
      Top = 153
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'tfrc'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 3
    end
    object DBEdit5: TDBEdit
      Left = 72
      Top = 195
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'zfje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 4
    end
    object DBEdit6: TDBEdit
      Left = 72
      Top = 236
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'zfrc'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 5
    end
    object DBEdit7: TDBEdit
      Left = 72
      Top = 279
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'ssje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 6
    end
    object dbedtmzzcf: TDBEdit
      Left = 104
      Top = 307
      Width = 63
      Height = 22
      Ctl3D = False
      DataField = 'mzzcf'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 7
    end
    object dbedtmzzcf4: TDBEdit
      Left = 92
      Top = 335
      Width = 75
      Height = 22
      Ctl3D = False
      DataField = 'xjzcf'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 8
    end
  end
  object Panel1: TPanel
    Left = 531
    Top = 40
    Width = 583
    Height = 434
    Align = alClient
    TabOrder = 8
    object Label1: TLabel
      Left = 10
      Top = 6
      Width = 96
      Height = 16
      Caption = #25910#25454#36215#27490#21495#65306
    end
    object Label2: TLabel
      Left = 10
      Top = 109
      Width = 96
      Height = 16
      Caption = #36864#36153#25910#25454#21495#65306
    end
    object Label3: TLabel
      Left = 10
      Top = 207
      Width = 88
      Height = 16
      Caption = #20316#24223#25910#25454#21495':'
    end
    object DBMemo1: TDBMemo
      Left = 6
      Top = 28
      Width = 327
      Height = 74
      Ctl3D = False
      DataField = 'sjqzh'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
    end
    object DBMemo2: TDBMemo
      Left = 10
      Top = 134
      Width = 327
      Height = 67
      Ctl3D = False
      DataField = 'tfsjh'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 1
    end
    object DBMemo3: TDBMemo
      Left = 10
      Top = 228
      Width = 327
      Height = 81
      Ctl3D = False
      DataField = 'zfsjh'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 2
    end
    object GroupBox4: TGroupBox
      Left = 353
      Top = 6
      Width = 175
      Height = 311
      Caption = #20854#20013':'#23601#35786#21345
      Ctl3D = False
      Font.Charset = GB2312_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = #23435#20307
      Font.Style = []
      ParentCtl3D = False
      ParentFont = False
      TabOrder = 3
      object Label28: TLabel
        Left = 8
        Top = 72
        Width = 60
        Height = 15
        Caption = #21150#21345#20154#27425
      end
      object Label29: TLabel
        Left = 8
        Top = 114
        Width = 75
        Height = 15
        Caption = #36864#23601#35786#21345#36153
      end
      object Label30: TLabel
        Left = 8
        Top = 156
        Width = 60
        Height = 15
        Caption = #36864#21345#20154#27425
      end
      object Label26: TLabel
        Left = 8
        Top = 30
        Width = 75
        Height = 15
        Caption = #25910#23601#35786#21345#36153
        Visible = False
      end
      object DBEdit18: TDBEdit
        Left = 88
        Top = 69
        Width = 82
        Height = 21
        DataField = 'jzksrrc'
        DataSource = ds_cxjz
        ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
        TabOrder = 0
      end
      object DBEdit19: TDBEdit
        Left = 88
        Top = 111
        Width = 82
        Height = 21
        DataField = 'jzktfje'
        DataSource = ds_cxjz
        ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
        TabOrder = 1
      end
      object DBEdit20: TDBEdit
        Left = 88
        Top = 153
        Width = 82
        Height = 21
        DataField = 'jzktfrc'
        DataSource = ds_cxjz
        ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
        TabOrder = 2
      end
      object DBEdit16: TDBEdit
        Left = 88
        Top = 26
        Width = 82
        Height = 21
        DataField = 'jzksfje'
        DataSource = ds_cxjz
        ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
        TabOrder = 3
        Visible = False
      end
    end
  end
  object GroupBox2: TGroupBox
    Left = 173
    Top = 40
    Width = 176
    Height = 434
    Align = alLeft
    Caption = #20854#20013#65306#38750#29616#37329
    TabOrder = 6
    object Label5: TLabel
      Left = 4
      Top = 31
      Width = 48
      Height = 16
      Caption = #24635#37329#39069
    end
    object Label6: TLabel
      Left = 4
      Top = 73
      Width = 48
      Height = 16
      Caption = #24635#20154#27425
    end
    object Label7: TLabel
      Left = 4
      Top = 116
      Width = 64
      Height = 16
      Caption = #36864#36153#37329#39069
    end
    object Label8: TLabel
      Left = 4
      Top = 158
      Width = 64
      Height = 16
      Caption = #36864#36153#20154#27425
    end
    object Label11: TLabel
      Left = 4
      Top = 200
      Width = 64
      Height = 16
      Caption = #20316#24223#37329#39069
    end
    object Label15: TLabel
      Left = 4
      Top = 240
      Width = 64
      Height = 16
      Caption = #20316#24223#20154#27425
    end
    object Label22: TLabel
      Left = 4
      Top = 279
      Width = 64
      Height = 16
      Caption = #38134#34892#21010#21345
    end
    object Label31: TLabel
      Left = 4
      Top = 312
      Width = 64
      Height = 16
      Caption = #38134#34892#36864#36153
    end
    object lbl2: TLabel
      Left = 0
      Top = 339
      Width = 80
      Height = 16
      Caption = #38134#34892#35786#26597#36153
    end
    object lbl3: TLabel
      Left = 0
      Top = 367
      Width = 80
      Height = 16
      Caption = #36716#36134#35786#26597#36153
    end
    object DBEdit8: TDBEdit
      Left = 72
      Top = 28
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'fxjsfje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 0
    end
    object DBEdit9: TDBEdit
      Left = 72
      Top = 70
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'fxjsfrc'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 1
    end
    object DBEdit10: TDBEdit
      Left = 72
      Top = 111
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'fxjtfje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 2
    end
    object DBEdit11: TDBEdit
      Left = 72
      Top = 153
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'fxjtfrc'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 3
    end
    object DBEdit12: TDBEdit
      Left = 72
      Top = 195
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'fxjzfje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 4
    end
    object DBEdit13: TDBEdit
      Left = 72
      Top = 236
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'fxjzfrc'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 5
    end
    object DBEdit14: TDBEdit
      Left = 72
      Top = 276
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'yhkhkje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 6
    end
    object DBEdit21: TDBEdit
      Left = 72
      Top = 310
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'yhktfje'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 7
    end
    object dbedtmzzcf1: TDBEdit
      Left = 73
      Top = 338
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'yhkzcf'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 8
    end
    object dbedtmzzcf2: TDBEdit
      Left = 73
      Top = 366
      Width = 85
      Height = 22
      Ctl3D = False
      DataField = 'zzzcf'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 9
    end
  end
  object cxButton1: TcxButton
    Left = 880
    Top = 7
    Width = 94
    Height = 29
    Caption = #25910#36153#26126#32454#25171#21360
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = #23435#20307
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    OnClick = cxButton1Click
  end
  object GroupBox3: TGroupBox
    Left = 349
    Top = 40
    Width = 182
    Height = 434
    Align = alLeft
    Caption = #20854#20013':'#31038#20445
    Ctl3D = False
    Font.Charset = GB2312_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = #23435#20307
    Font.Style = []
    ParentCtl3D = False
    ParentFont = False
    TabOrder = 7
    object Label16: TLabel
      Left = 8
      Top = 31
      Width = 45
      Height = 15
      Caption = #24635#37329#39069
    end
    object Label17: TLabel
      Left = 8
      Top = 73
      Width = 45
      Height = 15
      Caption = #24635#20154#27425
    end
    object Label18: TLabel
      Left = 8
      Top = 115
      Width = 60
      Height = 15
      Caption = #36864#36153#37329#39069
    end
    object Label21: TLabel
      Left = 8
      Top = 157
      Width = 60
      Height = 15
      Caption = #36864#36153#20154#27425
    end
    object Label23: TLabel
      Left = 8
      Top = 181
      Width = 38
      Height = 15
      Caption = #20854#20013':'
    end
    object Label25: TLabel
      Left = 8
      Top = 201
      Width = 60
      Height = 15
      Caption = #38376#35786#32479#31609
    end
    object Label27: TLabel
      Left = 8
      Top = 229
      Width = 60
      Height = 15
      Caption = #22235#33293#20116#20837
      Visible = False
    end
    object Label32: TLabel
      Left = 8
      Top = 257
      Width = 75
      Height = 15
      Caption = #31038#20445#30452#32467#25910
      Visible = False
    end
    object Label33: TLabel
      Left = 8
      Top = 284
      Width = 75
      Height = 15
      Caption = #31038#20445#30452#32467#36864
      Visible = False
    end
    object Label34: TLabel
      Left = 8
      Top = 312
      Width = 75
      Height = 15
      Caption = #36716#36134#27719#27454#25910
      Visible = False
    end
    object Label35: TLabel
      Left = 8
      Top = 338
      Width = 75
      Height = 15
      Caption = #36716#36134#27719#27454#36864
      Visible = False
    end
    object lbl4: TLabel
      Left = 9
      Top = 364
      Width = 75
      Height = 15
      Caption = #21307#20445#35786#26597#36153
    end
    object Label36: TLabel
      Left = 8
      Top = 390
      Width = 60
      Height = 15
      Caption = #23601#35786#21345#36153
    end
    object dbedt1: TDBEdit
      Left = 71
      Top = 28
      Width = 97
      Height = 21
      DataField = 'sbsrje'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 0
    end
    object dbedt2: TDBEdit
      Left = 71
      Top = 70
      Width = 97
      Height = 21
      DataField = 'sbsrrc'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 1
    end
    object dbedt3: TDBEdit
      Left = 71
      Top = 112
      Width = 97
      Height = 21
      DataField = 'sbtfje'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 2
    end
    object dbedt4: TDBEdit
      Left = 71
      Top = 154
      Width = 97
      Height = 21
      DataField = 'sbtfrc'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 3
    end
    object DBEdit15: TDBEdit
      Left = 71
      Top = 198
      Width = 97
      Height = 21
      DataField = 'mzgh_ghtcje'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 4
    end
    object DBEdit17: TDBEdit
      Left = 71
      Top = 226
      Width = 97
      Height = 21
      DataField = 'sswrje'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 5
      Visible = False
    end
    object DBEdit22: TDBEdit
      Left = 94
      Top = 255
      Width = 77
      Height = 21
      DataField = 'fxjsbsf'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 6
      Visible = False
    end
    object DBEdit23: TDBEdit
      Left = 94
      Top = 282
      Width = 77
      Height = 21
      DataField = 'fxjsbtf'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 7
      Visible = False
    end
    object DBEdit24: TDBEdit
      Left = 94
      Top = 310
      Width = 77
      Height = 21
      DataField = 'fxjzzsf'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 8
      Visible = False
    end
    object DBEdit25: TDBEdit
      Left = 94
      Top = 335
      Width = 77
      Height = 21
      DataField = 'fxjzztf'
      DataSource = ds_cxjz
      ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
      TabOrder = 9
      Visible = False
    end
    object dbedtmzzcf3: TDBEdit
      Left = 94
      Top = 362
      Width = 78
      Height = 21
      Ctl3D = False
      DataField = 'ybzcf'
      DataSource = ds_cxjz
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 10
    end
    object db_jzkf: TDBEdit
      Left = 94
      Top = 387
      Width = 78
      Height = 21
      Ctl3D = False
      DataField = 'jzkf'
      DataSource = ds_jzkf
      ImeName = #29579#30721#20116#31508#22411#36755#20837#27861'86'#29256
      ParentCtl3D = False
      ReadOnly = True
      TabOrder = 11
    end
  end
  object DBLookupComboBox1: TDBLookupComboBox
    Left = 307
    Top = 8
    Width = 186
    Height = 24
    ImeName = #20013#25991' - QQ'#25340#38899#36755#20837#27861
    KeyField = 'jzdh'
    ListField = 'jzdh'
    ListSource = ds_cxjz
    TabOrder = 4
    OnClick = DBLookupComboBox1Click
  end
  object zdybb: TCheckBox
    Left = 687
    Top = 46
    Width = 106
    Height = 19
    Caption = #33258#23450#20041#25253#34920
    Checked = True
    State = cbChecked
    TabOrder = 9
    Visible = False
  end
  object chk_jzrq: TCheckBox
    Left = 499
    Top = 10
    Width = 130
    Height = 17
    Caption = #25353#32467#36134#26085#26399#25171#21360
    TabOrder = 10
  end
  object chk_Allczy: TCheckBox
    Left = 635
    Top = 11
    Width = 134
    Height = 17
    Caption = #25171#21360#25152#26377#25805#20316#21592
    TabOrder = 11
    OnClick = chk_AllczyClick
  end
  object ds_cxjz: TDataSource
    DataSet = qry_cxjz
    Left = 546
    Top = 294
  end
  object qry_cxjz: TADOQuery
    Connection = DM_data.ado_mydata
    CursorType = ctStatic
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'czydm'
        DataType = ftString
        Size = 1
        Value = Null
      end
      item
        Name = 'jzrq'
        DataType = ftDateTime
        Size = 1
        Value = Null
      end>
    SQL.Strings = (
      'select * from mzsf_czyjz '
      'where czydm=:czydm and'
      ' convert(char(10),jzrq,102)=convert(char(10),:jzrq,102)')
    Left = 514
    Top = 294
  end
  object qry_cwkjzrq: TADOQuery
    Connection = DM_data.ado_mydata
    Parameters = <
      item
        Name = 'jzdh'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = Null
      end
      item
        Name = 'czydm'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      
        'select convert(char(10),ksrq,102)+space(2)+convert(char(10),ksrq' +
        ',108) ksrq,'
      
        '       convert(char(10),jsrq,102)+space(2)+convert(char(10),jsrq' +
        ',108) jsrq'
      ' from mzsf_cwkjz'
      ' where jzdh=:jzdh   and'
      '       czydm=:czydm')
    Left = 579
    Top = 294
  end
  object qry_dy: TADOQuery
    Connection = DM_data.ado_mydata
    Parameters = <
      item
        Name = 'czydm'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 4
        Value = Null
      end
      item
        Name = 'jzdh'
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 16
        Value = Null
      end>
    SQL.Strings = (
      
        'select *,fxjsfje-(fxjtfje+fxjzfje) kzf,ssje+(sbsrje-sbtfje) ssje' +
        '1'
      'from mzsf_czyjz where czydm=:czydm and jzdh=:jzdh'
      '')
    Left = 615
    Top = 294
  end
  object sp_jzdy: TADOStoredProc
    Connection = DM_data.ado_mydata
    ProcedureName = 'mzsf_in_czyrb;1'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
        Value = Null
      end
      item
        Name = '@czydm'
        Attributes = [paNullable]
        DataType = ftString
        Size = 4
        Value = Null
      end
      item
        Name = '@jzdh'
        Attributes = [paNullable]
        DataType = ftString
        Size = 16
        Value = Null
      end>
    Left = 288
    Top = 112
  end
  object qry_dy_sfmx: TADOQuery
    Connection = DM_data.ado_mydata
    Parameters = <
      item
        Name = 'czydm'
        Attributes = [paNullable]
        DataType = ftString
        NumericScale = 255
        Precision = 255
        Size = 4
        Value = Null
      end>
    SQL.Strings = (
      'select *  from mzsf_czyrb where czydm=:czydm'
      '')
    Left = 338
    Top = 104
  end
  object qry_cxjz_sfmx: TADOQuery
    Connection = DM_data.ado_mydata
    LockType = ltReadOnly
    Parameters = <
      item
        Name = 'czydm'
        Size = -1
        Value = Null
      end
      item
        Name = 'jzrq'
        Size = -1
        Value = Null
      end>
    SQL.Strings = (
      'select *,fxjsfje - fxjtfje - fxjzfje fxjssje from mzsf_czyjz '
      'where czydm=:czydm and'
      ' convert(char(10),jzrq,102)=convert(char(10),:jzrq,102)'
      '')
    Left = 232
    Top = 104
  end
  object qry_sbjz: TADOQuery
    Connection = DM_data.ado_mydata
    CursorType = ctStatic
    LockType = ltBatchOptimistic
    Parameters = <>
    SQL.Strings = (
      'select * from sbjk_mzsf_czyjz ')
    Left = 384
    Top = 104
  end
  object qry_tsbrfy: TADOQuery
    Connection = DM_data.ado_mydata
    CursorType = ctStatic
    Parameters = <>
    SQL.Strings = (
      'SELECT * FROM dbo.mzsf_tsbrfymx WHERE 1=2')
    Left = 176
    Top = 104
  end
  object sp_maindata: TADOStoredProc
    Connection = DM_data.ado_mydata
    ProcedureName = 'zdybb_cx_mzsrrbb'
    Parameters = <
      item
        Name = '@jzrq'
        Attributes = [paNullable]
        DataType = ftDateTime
        Value = Null
      end
      item
        Name = '@jzdh'
        Attributes = [paNullable]
        DataType = ftString
        Size = -1
        Value = Null
      end>
    Left = 896
    Top = 256
  end
  object sp_mxdata: TADOStoredProc
    Connection = DM_data.ado_mydata
    ProcedureName = 'zdybb_cx_mzsrrbb1'
    Parameters = <
      item
        Name = '@jzrq'
        Attributes = [paNullable]
        DataType = ftDateTime
        Value = Null
      end
      item
        Name = '@jzdh'
        Attributes = [paNullable]
        DataType = ftString
        Size = -1
        Value = Null
      end>
    Left = 952
    Top = 256
  end
  object sp_jzkf: TADOStoredProc
    Connection = DM_data.ado_mydata
    ProcedureName = 'mzsf_cx_mzjzqk'
    Parameters = <
      item
        Name = '@RETURN_VALUE'
        DataType = ftInteger
        Direction = pdReturnValue
        Precision = 10
      end
      item
        Name = '@jzdh'
        Attributes = [paNullable]
        DataType = ftString
        Size = 16
      end>
    Left = 560
    Top = 96
  end
  object ds_jzkf: TDataSource
    DataSet = sp_jzkf
    Left = 600
    Top = 96
  end
end
