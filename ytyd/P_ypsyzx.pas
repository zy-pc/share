...
begin
 ...
 // treeview1.Enabled:=false;
    ...
    qry_find.Close;
    qry_find.Parameters.ParamValues['dm'] := dm;
    qry_find.Open;

    if qry_find.RecordCount <= 0 then
    begin
    ...
    else if qry_find.RecordCount >= 2 then
    begin
        dbgrid1.Visible := true;
        dbgrid1.SetFocus;
    end
    else if qry_find.RecordCount = 1 then
    begin
        item_seek(qry_find['lbdm'], qry_find['ypdm']);
    end;
end;

procedure Tfrm_yyzx.DBGrid1DblClick(Sender: TObject);
begin
    edit1.Text := qry_find['ypdm'];
    DBGrid1.Visible := false;
    item_seek(qry_find['lbdm'], qry_find['ypdm']);
end;
...
begin
    ...
    conn.ConnectionString := ado_connet_string('sys_xyyd',1);
    conn.Connected := true;
end;
