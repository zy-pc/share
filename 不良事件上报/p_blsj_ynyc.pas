...
type
  Tfrm_ynyc = class(TForm)
    ...
    cxGrid1DBTableView1ycfqmc: TcxGridDBColumn;
    sp_sjcxycbwmc: TStringField;
    cxGrid1DBTableView1ycbwmc: TcxGridDBColumn;
...
implementation
uses p_dm,p_func,p_tjbg,p_blsj_checkError,p_main,p_ynycjprint,p_blsj_gwyspf,p_blsj_bqgcyy;

{$R *.dfm}

procedure Tfrm_ynyc.Button10Click(Sender: TObject);
var
...
begin
  ...
  if ra_fq.Checked then
  tjlx := '1'
  else if ra_bw.Checked then
  tjlx := '2';
...
procedure Tfrm_ynyc.Button12Click(Sender: TObject);
begin
...
procedure Tfrm_ynyc.Button13Click(Sender: TObject);
begin
 if SaveDialog1.Execute then
  begin
...
procedure Tfrm_ynyc.Button1Click(Sender: TObject);
var
s:string;
i:Integer;
...
xzbz:='2';
ado_jbxx.Close;
ado_jbxx.CommandText:='select * from blsj_ynyc_jbxx where id='+#39+id+#39+' order by id';
ado_jbxx.Open;
...
qry_temp.Close;
qry_temp.SQL.Clear;
qry_temp.sql.Add('select  * from blsj_ynyc_cljl where sjid=' + #39 + id + #39 +' and cljb='+#39+inttostr(strtoint(cljb)+1)+#39+ ' and thbz=1 order by thrq desc');
qry_temp.Open;
...
if not ljjc then
 begin
  qry_temp.Close;
  qry_temp.SQL.Clear;
  qry_temp.SQL.Add('select * from blsj_ynyc_jbxx where id='+id);
  qry_temp.Open;
...
procedure Tfrm_ynyc.Button2Click(Sender: TObject);
var
ls,qjks,jlid:string;
begin
if not ljjc then
begin
 ...
 begin
 button5.Click;
 ls:='update blsj_ynyc_jbxx set sfwc=1 where id='+#39+id+#39 ;
 qry_temp.Close;
 qry_temp.SQL.Clear;
 qry_temp.SQL.Add(ls);
 qry_temp.ExecSQL;
...
procedure Tfrm_ynyc.Button3Click(Sender: TObject);
var
...
begin
application.CreateForm(Tynycprint,ynycprint);
try
 ynycprint.QRLabel17.Caption:=pub_yymc+'院内压疮报告表';
 ynycprint.QRLabel25.Caption:=Trim(Editxm.Text);
 ynycprint.QRLabel2.Caption:=Trim(DBEdit4.Text);
 ynycprint.QRLabel26.Caption:=Trim(DBEdit9.Text);
 ynycprint.QRLabel44.Caption:=Trim(edit1.Text);
 ynycprint.QRLabel149.Caption:=Trim(dbedit5.Text);
 ynycprint.QRLabel147.Caption:=Trim(combobox9.Text);
 ynycprint.QRLabel13.Caption:=trim(dbedit6.Text)+''+Trim(jbmc.Caption);
 ynycprint.QRLabel20.Caption:=Trim(dbedit12.Text); //压疮治愈护士长复核签名
 ynycprint.QRLabel48.Caption:=Trim(dbedit7.Text);//申报人签名
 ynycprint.QRLabel45.Caption:=Trim(dbedit3.Text); //护士长签名
 ynycprint.QRLabel8.Caption:=Trim(dbedit8.Text); //Branden评分
 if FormatDateTime('yyyy-mm-dd ',dtpryrq.Date)>'1900-00-00' then //入院时间
 ynycprint.QRLabel15.Caption:=FormatDateTime('yyyy年mm月dd日',dtpryrq.Date);
 if FormatDateTime('yyyy-mm-dd ',dtphsz.Date)>'1900-00-00' then
 ynycprint.QRLabel50.Caption:=FormatDateTime('yyyy年mm月dd日 hh小时mm分',dtphsz.Date);
 ynycprint.QRLabel47.Caption:=Trim(dbedit10.text); //复核者签名
 if FormatDateTime('yyyy-mm-dd ',dtpfhz.Date)>'1900-00-00' then //复核时间
 ynycprint.QRLabel52.Caption:=FormatDateTime('yyyy年mm月dd日 hh小时mm分',dtpfhz.Date);
 if FormatDateTime('yyyy-mm-dd ',dtpckrq.Date)>'1900-00-00' then //出科时间
 ynycprint.QRLabel91.Caption:=FormatDateTime('yyyy年mm月dd日 ',dtpckrq.Date);
 ynycprint.QRLabel33.Caption:=Trim(dbedit13.Text); //接收科室复核签名
 if FormatDateTime('yyyy-mm-dd ',dtpjsqmrq.Date)>'1900-00-00' then//接收科室签名日期
 ynycprint.QRLabel35.Caption:=FormatDateTime('yyyy年mm月dd日',dtpjsqmrq.Date);
 ynycprint.QRLabel151.Caption:=Trim(dbedit11.Text);
 if trim(combobox1.Text)='是' then
 ynycprint.QRLabel160.Caption:='√'
 else
 if trim(combobox1.Text)='否' then
 ynycprint.QRLabel158.Caption:='√';
 if FormatDateTime('yyyy-mm-dd ',dtpnmyc.Date)>'2000-00-00' then //压疮申报时间
 ynycprint.QRLabel162.Caption:=FormatDateTime('yyyy年mm月dd日',dtpnmyc.Date);
 if FormatDateTime('yyyy-mm-dd ',dtpfsrq.Date)>'2000-00-00' then //压疮发生日期
 ynycprint.QRLabel163.Caption:=FormatDateTime('yyyy年mm月dd日 hh时',dtpfsrq.Date);
 if FormatDateTime('yyyy-mm-dd ',dtpsbrq.Date)>'2000-00-00' then //上报日期
 ynycprint.QRLabel38.Caption:=FormatDateTime('yyyy年mm月dd日 hh时',dtpsbrq.Date);
 if trim(combobox10.Text)='转科' then //出科类别
 begin
 ynycprint.QRLabel145.Caption:='√';
 ynycprint.QRLabel95.Caption:=trim(combobox8.text);
 end
 else if ComboBox10.Text='出院' then
 ynycprint.QRLabel144.Caption:='√'
 else if ComboBox10.Text='死亡' then
 ynycprint.QRLabel97.Caption:='√';
 if trim(combobox2.Text)='治愈' then //压疮转归情况
 ynycprint.QRLabel21.Caption:='√'
 else if trim(combobox2.Text)='好转' then
 ynycprint.QRLabel27.Caption:='√'
 else if trim(combobox2.Text)='无变化' then
 ynycprint.QRLabel29.Caption:='√'
 else if trim(combobox2.Text)='恶化' then
 ynycprint.QRLabel31.Caption:='√';
 if FormatDateTime('yyyy-mm-dd ',dtpckrq.Date)>'1900-00-00' then //出科日期
 ynycprint.QRLabel91.Caption:=FormatDateTime('yyyy年mm月dd日',dtpckrq.Date);
 if trim(dbedit13.Text)<>'' then //接收科室复核签名
 begin
  ynycprint.QRLabel33.Caption:=trim(dbedit13.Text);
  if FormatDateTime('yyyy-mm-dd ',dtpjsqmrq.Date)>'1900-00-00' then //接收科室复核日期
  ynycprint.QRLabel35.Caption:=FormatDateTime('yyyy年mm月dd日',dtpjsqmrq.Date);
 end;
 if trim(dbedit12.Text)<>'' then //护士长复核签名
 ynycprint.QRLabel20.Caption:=trim(dbedit12.Text);
 if (FormatDateTime('yyyy-mm-dd ',dtpzyrq.Date)>'1900-00-00') and (trim(combobox2.Text)='治愈')then //治愈日期
 ynycprint.QRLabel18.Caption:=FormatDateTime('yyyy年mm月dd日',dtpzyrq.Date);
 ynycprint.QRLabel151.Caption:=trim(dbedit11.Text);//出院信息护士签名
...
procedure Tfrm_ynyc.Button3Exit(Sender: TObject);
begin
groupbox4.Enabled:=false;;
end;

procedure Tfrm_ynyc.Button4Click(Sender: TObject);
begin
Self.Close;
end;

procedure Tfrm_ynyc.Button5Click(Sender: TObject);
begin
  if cljb<>'3' then
  begin
    ado_jbxx['hzqsqk']:=Memo1.Text;
    ado_jbxx['yczk']:=Memo2.Text;
    ado_jbxx['mqhlcs']:=Memo3.Text;
  end;
  ado_jbxx['skwyhyj']:=Memo4.Text;
  ado_jbxx['sfhz']:=Trim(ComboBox5.Text);
  xzxbc;//选择项保存以及压疮治愈自动提交复核护士长
  ado_jbxx.Post;
  qkjm;//清空界面
  sxlb;
...
procedure Tfrm_ynyc.Button6Click(Sender: TObject);
var
ls,qjks,clczy,jlid,tjks,tjczy,thyy:string;
begin
 dqks:=Trim(ado_jbxx.fieldbyname('ksdm').asstring);
 dqczy:=Trim(ado_jbxx.fieldbyname('clczy').asstring);
 ls := 'select top 1 * from blsj_ynyc_cljl where sjid=' + #39 + id + #39 + ' and (thbz<>1  or thbz is null) order by id desc';
 qry_temp.Close;
 qry_temp.SQL.Clear;
 qry_temp.SQL.Add(ls);
 qry_temp.Open;
 ...
 ls:='update blsj_ynyc_jbxx set ksdm='+#39+qjks+#39+',clczy='+#39+clczy+#39+',cljb='+#39+cljb+#39+' where id='+#39+id+#39;
 qry_temp.Close;
 qry_temp.SQL.Clear;
 qry_temp.SQL.Add(ls);
 qry_temp.ExecSQL;
 ls := 'update   blsj_ynyc_cljl set thbz=1,thrq=Getdate(),thry='+#39+pub_czydm+#39+',thyy='+#39+thyy+#39+' where id=' + #39 + jlid + #39;
 qry_temp.Close;
 qry_temp.SQL.Clear;
 qry_temp.SQL.Add(ls);
 qry_temp.ExecSQL;
...
procedure Tfrm_ynyc.Button7Click(Sender: TObject);
begin
Self.Close;
end;

procedure Tfrm_ynyc.Button8Click(Sender: TObject);
begin
qkjm;
sxlb;
groupbox4.Enabled:=true;
...
procedure Tfrm_ynyc.Button9Click(Sender: TObject);
var
ls,ksrq,jsrq:string;
begin
...
begin
 ...
 sp_sjcx.parameters.ParamByName('@sjlx').value:='5';
...
procedure Tfrm_ynyc.ComboBox10KeyPress(Sender: TObject; var Key: Char);
begin
key := #0;
end;

procedure Tfrm_ynyc.ComboBox1DblClick(Sender: TObject);
var
zyh,sbrq:string;
begin
...
qry_temp.Close;
qry_temp.SQL.Clear;
qry_temp.SQL.Add('select top 1 * from blsj_ycsj_jbxx where zyh='+#39+zyh+#39+' order by sbrq desc' );
qry_temp.Open;
if qry_temp.RecordCount>0 then
begin
combobox1.Text:='是';
sbrq:=trim(qry_temp.FieldByName('sbrq').AsString);
ado_jbxx['gwycbgrq']:=trim(qry_temp.FieldByName('sbrq').AsString);
ado_jbxx['pgzf']:=trim(qry_temp.FieldByName('pgzf').AsString);
end
else
begin
combobox1.Text:='否';
Application.MessageBox('没有找到该病人的申报记录', '错误提示', 16)
end;
end;

procedure Tfrm_ynyc.ComboBox2Exit(Sender: TObject);
var
s:string;
begin
...
procedure Tfrm_ynyc.ComboBox2KeyPress(Sender: TObject; var Key: Char);
begin
key := #0;
end;

procedure Tfrm_ynyc.ComboBox3Change(Sender: TObject);
var s:string;
begin
...
procedure Tfrm_ynyc.ComboBox3Exit(Sender: TObject);
var
s:string;
begin
s:=Trim(ComboBox3.Text);
...
procedure Tfrm_ynyc.ComboBox3KeyPress(Sender: TObject; var Key: Char);
begin
key := #0;
end;

procedure Tfrm_ynyc.ComboBox4Exit(Sender: TObject);
var
s:string;
begin
...
procedure Tfrm_ynyc.ComboBox4KeyPress(Sender: TObject; var Key: Char);
begin
key := #0;
end;

procedure Tfrm_ynyc.ComboBox6Exit(Sender: TObject);
begin
  if combobox6.Text='治愈' then
 begin
  application.CreateForm(Tfrm_tjbg, frm_tjbg);
  frm_tjbg.sjid:='S'+id;
  frm_tjbg.ShowModal;
...
procedure Tfrm_ynyc.ComboBox7KeyPress(Sender: TObject; var Key: Char);
begin
if Key=#13 then
Memo1.SetFocus;
end;

procedure Tfrm_ynyc.ComboBox9Exit(Sender: TObject);
var
s,s1,ls:string;
begin
kj1:=Sender;
...
procedure Tfrm_ynyc.cxGrid1DBTableView1CellDblClick(
...
procedure Tfrm_ynyc.cxGrid1DBTableView1CustomDrawCell(
...
procedure Tfrm_ynyc.DBEdit1Exit(Sender: TObject);
var s1,ls:string;
begin
...
procedure Tfrm_ynyc.DBEdit3DblClick(Sender: TObject);
begin
qry_temp.Close;
 qry_temp.SQL.Clear;
 qry_temp.sql.Add('select  * from blsj_ynyc_cljl where sjid=' + #39 + id + #39 +' and cljb='+#39+inttostr(strtoint(cljb)+1)+#39+ ' and thbz=1 order by thrq desc');
...
procedure Tfrm_ynyc.DBGrid1DblClick(Sender: TObject);
begin
 if kj1=DBEdit1 then
  begin
...
procedure Tfrm_ynyc.DBGrid1KeyPress(Sender: TObject; var Key: Char);
begin
 if key=#13 then
 begin
  if kj1=DBEdit1 then
...
procedure Tfrm_ynyc.DBGrid3CellClick(Column: TColumn);
var
sfwc,sfck,jsksqrbz:string;
begin
  qkjm;
...
procedure Tfrm_ynyc.DBGrid3DblClick(Sender: TObject);
var
sfwc,sfck,jsksqrbz:string;
begin
{qkjm;
...
procedure Tfrm_ynyc.DBGrid3DrawColumnCell(Sender: TObject; const Rect: TRect;
  DataCol: Integer; Column: TColumn; State: TGridDrawState);
begin
...
procedure Tfrm_ynyc.DBGrid4DblClick(Sender: TObject);
var
...
begin
 ...
 ryrq:=trim(sp_cx.FieldByName('brch').AsString);
 ryrq:=trim(sp_cx.FieldByName('zyh').AsString);
 brks:=trim(sp_cx.FieldByName('brks').AsString);
 ls:='insert into blsj_ynyc_jbxx(tmh,brxm,brxb,brnl,sbks,sbr,clczy,ksdm,cljb,sfwc,ryrq,sbrq,brch,zyh,brks) select '+#39+bybh+#39+','+#39+brxm+#39+','+#39+brxb+#39+','+#39+brnl+#39+','+#39+pub_ksdm+#39+','+#39+pub_czyxm+#39+','+#39+pub_czydm+#39+','+#39+pub_ksdm+#39+','+#39+'1'+#39+','+#39+'0'+#39+','+#39+ryrq+#39+' ,GETDATE()'+','+#39+brch+#39+','+#39+zyh+#39+','+#39+brks+#39;
 qry_insert.Close;
 qry_insert.SQL.Clear;
 qry_insert.SQL.Add(ls);
 qry_insert.ExecSQL;
 ado_jbxx.Close;
 ado_jbxx.CommandText:='select  top 1 * from blsj_ynyc_jbxx where tmh='+#39+bybh+#39+' order by id desc';
 ado_jbxx.Open;
 ado_jbxx.Edit;
 sjcs;
 ls:='select * from blsj_ynyc_jbxx where  (sfwc<>1 and clczy='+#39+pub_czydm+#39+') or (ksdm='+#39+pub_ksdm+#39+' and sfwc<>1 and clczy='+#39+#39+')';
 qry_lb.Close;
 qry_lb.SQL.Clear;
 qry_lb.SQL.Add(ls);
 qry_lb.Open;
...
procedure Tfrm_ynyc.DBGrid4KeyPress(Sender: TObject; var Key: Char);
var
ls,bybh,brxm,brxb,brnl,sfwc,ryrq,sbr,brch,zyh,brks:string;
begin
if Key=#13 then
...
procedure Tfrm_ynyc.DBGrid5DblClick(Sender: TObject);
var
sfwc,sfck,jsksqrbz:string;
begin
pc1.ActivePage:=TabSheet1;
...
procedure Tfrm_ynyc.dtpckrqEnter(Sender: TObject);
begin
 gbbz:=GroupBox4.Enabled;
 GroupBox4.Enabled:=True;
end;

procedure Tfrm_ynyc.dtpckrqExit(Sender: TObject);
begin
  GroupBox4.Enabled:=gbbz;
end;

procedure Tfrm_ynyc.edit1KeyPress(Sender: TObject; var Key: Char);
var
...
if Key=#13 then
 ...
 if tmh<>'' then
 begin
  ls := 'select * from blsj_ynyc_jbxx where tmh=' + #39 + tmh + #39 + ' or brxm=' + #39 + tmh + #39;
  ls :=ls+' union   select a.* from blsj_ynyc_jbxx a,zysf_zydj b where b.bqdm='+pub_bqdm+'  and rtrim(ltrim(a.tmh))=rtrim(ltrim(b.tmh))  and a.brch='+#39+tmh+#39 +' and a.sbks='+#39+pub_ksdm+#39;
  qry_temp.Close;
  qry_temp.SQL.Clear;
  qry_temp.SQL.Add(ls);
  qry_temp.Open;
    ...
    if (application.MessageBox('已存在该卡号或姓名上报信息，是否继续？','请注意',MB_YESNO + MB_ICONQUESTION) =IDYES) then
     begin
       ...
       begin
        ...
        ls:='insert into blsj_ynyc_jbxx(tmh,brxm,brxb,brnl,sbks,sbr,clczy,ksdm,cljb,sfwc,ryrq,sbrq,brch,zyh,brks,ryzd) select '+#39+bybh+#39+','+#39+brxm+#39+','+#39+brxb+#39+','+#39+brnl+#39+','+#39+pub_ksdm+#39+','+#39+pub_czyxm+#39+','+#39+pub_czydm+#39+','+#39+pub_ksdm+#39+','+#39+'1'+#39+','+#39+'0'+#39+','+#39+ryrq+#39+' ,GETDATE()'+','+#39+brch+#39+','+#39+zyh+#39+','+#39+brks+#39+','+#39+ryzd+#39;
        qry_insert.Close;
        qry_insert.SQL.Clear;
        qry_insert.SQL.Add(ls);
        qry_insert.ExecSQL;
  ...
  begin
   ...
   if sp_cx.RecordCount=0 then  //没有该卡号挂号记录 或 住院记录
    ...
    begin
     ...
     qry_insert.ExecSQL;
     ado_jbxx.Close;
     ado_jbxx.CommandText:='select * from blsj_ynyc_jbxx where tmh='+#39+bybh+#39+' order by id desc';
     ado_jbxx.Open;
     ado_jbxx.Edit;
     sjcs;
     ls:='select * from blsj_ynyc_jbxx where  (sfwc<>1 and clczy='+#39+pub_czydm+#39+') or (ksdm='+#39+pub_ksdm+#39+' and sfwc<>1 and clczy='+#39+#39+')';
...
procedure Tfrm_ynyc.FormActivate(Sender: TObject);
begin
edit1.SetFocus
end;

procedure Tfrm_ynyc.FormCreate(Sender: TObject);
var
ls:string;
x,y:Integer;
begin
...
procedure Tfrm_ynyc.FormKeyPress(Sender: TObject; var Key: Char);
begin
 if (key=#13) and (Memo1.Focused=False) and (dtpckrq.Focused=False) then
 SelectNext(activeControl,true,true);
end;

procedure Tfrm_ynyc.FormShow(Sender: TObject);
begin
dtpksrq.Date:=Now;
dtpjsrq.Date:=Now;
dtptjs.Date:=Now;
...
procedure Tfrm_ynyc.Add_xsz(bmc,cxz:string;Cbox:TComboBox);
var sj:string;
begin
  Qry_pub.close;
  Qry_pub.SQL.clear;
 ...
 function Tfrm_ynyc.get_xsz(bmc,cxz:string):string;
var sj:string;
begin
  Qry_pub.close;
  Qry_pub.SQL.clear;
 ...
 if Trim(DBEdit6.Text)='' then
  ...
  if (Trim(DBEdit8.Text)='') and (trim(combobox1.Text)='是') then
  begin
   iii:=iii+1;
  ...
  if trim(combobox1.Text)='' then
   begin
    iii:=iii+1;
    frm_blsj_checkerror.Memo1.Lines.Add(trim(inttostr(iii))+'：是否申报难免压疮不能为空');
   end;
 ...
 procedure Tfrm_ynyc.Memo1DblClick(Sender: TObject);
 begin
 str_mbct:='';
...
procedure Tfrm_ynyc.Memo2DblClick(Sender: TObject);
begin
str_mbct:='';
if (cljb='1') or (cljb='') then
begin
...
procedure Tfrm_ynyc.Memo3DblClick(Sender: TObject);
begin
str_mbct:='';
if (cljb='1') or (cljb='') then
begin
...
procedure Tfrm_ynyc.MenuItem1Click(Sender: TObject);
var
ls,qjks,clczy,jlid,tjks,tjczy,thyy:string;
begin
 dqks:=Trim(ado_jbxx.fieldbyname('ksdm').asstring);
...
procedure Tfrm_ynyc.N1Click(Sender: TObject);
 var
...
begin
 ...
 or ((cklb='转科') and (yczkqk<>'') and (yczkqk='治愈') and (cyxxhsqm<>'')  and (ckrq>'2000'))
 then
 begin
 sid:=trim(qry_lb.FieldByName('id').AsString);
 if trim(edit1.Text)<>'' then
 button5.Click;
 ls:='update blsj_ynyc_jbxx set sfck=1 where id ='+#39+sid+#39;
 qry_temp.Close;
 qry_temp.SQL.Clear;
 qry_temp.SQL.Add(ls);
 qry_temp.ExecSQL;
...
procedure Tfrm_ynyc.N2Click(Sender: TObject);
var
...
begin
 ...
 if (jsksfhqm<>'')  and (jsqmrq>'2000')  then
  begin
   ...
   if trim(edit1.Text)<>'' then
   button5.Click;
   ls:='update blsj_ynyc_jbxx set jsksqrbz=1 where id ='+#39+sid+#39;
   qry_temp.Close;
   qry_temp.SQL.Clear;
   qry_temp.SQL.Add(ls);
   qry_temp.ExecSQL;
 ...
 application.MessageBox(pchar('转出科室未确认病人出科,请核查!'),'注意',16);
end;

procedure Tfrm_ynyc.sjcs;//查询信息时实际传送值
 var fsrq,dhtzrq,s:string;
 begin
 if ado_jbxxtmh.asstring<>'' then
 edit1.Text:=Trim(ado_jbxx.fieldbyname('tmh').asstring);
 if ado_jbxxhzqsqk.asstring<>'' then
 Memo1.Text:=ado_jbxx.fieldbyname('hzqsqk').asstring;
 ...
 ComboBox1.Text:=trim(ado_jbxx.fieldbyname('sffsyc').asstring);
...
procedure Tfrm_ynyc.sp_sjcxCalcFields(DataSet: TDataSet);
var
s:string;
begin
 s:=Trim(sp_sjcx.fieldbyname('sfwc').asstring); //完成状态
 if s='1' then
 sp_sjcx['lcsfwc']:='已完成'
 else if s='0' then
 sp_sjcx['lcsfwc']:='未完成';
 sp_sjcx['brly']:='住院';
 s:=Trim(sp_sjcx.fieldbyname('sbks').asstring);//上报科室
 s:=get_xsz('sys_ksdm',s);
 sp_sjcx['sbksmc']:=s;

 s:=Trim(sp_sjcx.fieldbyname('clczy').asstring);//最后处理人员
 s:=get_xsz('sys_czy',s);
 sp_sjcx['zhclry']:=s;

 s:=Trim(sp_sjcx.fieldbyname('ksdm').asstring); //最后处理科室
 s:=get_xsz('sys_ksdm',s);
 sp_sjcx['zhclks']:=s;
...
procedure Tfrm_ynyc.xzxbc;
begin
  ado_jbxx['sffsyc']:=trim(combobox1.Text);
...
begin
 ...
 procedure Tfrm_ynyc.qry_lbAfterDelete(DataSet: TDataSet);
 var
 ls:string;
 begin
 qkjm;
 ...
 procedure Tfrm_ynyc.cljbqxkz(cljb: string;sfwc:string;sfck:string;jsksqrbz:string);
 begin
  if  cljb='1' then
  begin
   groupbox4.Enabled:=true;
