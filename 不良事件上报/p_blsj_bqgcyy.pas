...
if (not qry_yygf.IsEmpty)  and (sjlb='护理不良事件') then
 ...
 frm_ynyc.str_mbct:=qry_yygf.FieldByName('yy').AsString
  else if (not qry_yygf.IsEmpty)  and (sjlb='事件经过') then
 frm_ylaqsjbgb.str_mbct:=qry_yygf.FieldByName('yy').AsString
  else if (not qry_yygf.IsEmpty)  and (sjlb='导致事件的原因') then
 frm_ylaqsjbgb.str_mbct:=qry_yygf.FieldByName('yy').AsString
  else if (not qry_yygf.IsEmpty)  and (sjlb='事件处理情况') then
 frm_ylaqsjbgb.str_mbct:=qry_yygf.FieldByName('yy').AsString
  else if (not qry_yygf.IsEmpty)  and (sjlb='不良事件评价') then
 frm_ylaqsjbgb.str_mbct:=qry_yygf.FieldByName('yy').AsString
  else if (not qry_yygf.IsEmpty)  and (sjlb='持续改进措施') then
 frm_ylaqsjbgb.str_mbct:=qry_yygf.FieldByName('yy').AsString
...
begin
   TreeView1.Items.Clear ;
       ...
       ADOQuery1.close;
       ADOQuery1.SQL.clear;
       ADOQuery1.SQL.Add(' select distinct lb from blsj_ctmb where   lb=:lb or txxm=:lb1');
       ADOQuery1.Parameters.ParamByName('lb').Value:=sjlb;
       ADOQuery1.Parameters.ParamByName('lb1').Value:=sjlb;
       ADOQuery1.open;
       ADOQuery1.first;
 ...
 for i := 0 to ADOQuery1.RecordCount - 1 do
       ...
       qry_yygf.close;
       qry_yygf.sql.clear;
       qry_yygf.SQL.add('select * from blsj_ctmb where   lb=:lb or txxm=:lb1 ');
       qry_yygf.Parameters.ParamByName('lb').Value:=sjlb;
       qry_yygf.Parameters.ParamByName('lb1').Value:=sjlb;
       qry_yygf.Open;
