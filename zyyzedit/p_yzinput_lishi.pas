...
uses
  ...
  cxButtons, DBGridEhGrouping, OleCtrls, SHDocVw, RzEdit;
type
  Tmyrect = record
    ph: string;
...
type
  Tfrm_yzinput_lishi = class( TForm )
    ...
    procedure tab_pacsShow(Sender: TObject);
    procedure Edt_pass_tmhKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure Button17Click(Sender: TObject);
    procedure btnSFLISClick(Sender: TObject);
    procedure btnJCSFClick(Sender: TObject);
    procedure btn_gtyxClick(Sender: TObject);
...
uses p_dm, p_func, p_yzzt, p_treeview_ctxz, p_treeview_ctwh, p_bylsyz, p_tjyz, p_yzfz,
  ...
  p_lsyzxzdy, p_jcsqd, p_dslr, p_zxdxd,p_xgyl,p_xbs,p_pub_sj,ShellAPI;
...
procedure Tfrm_yzinput_lishi.tab_pacsShow(Sender: TObject);
var
  visitAddr: string;
begin
  ...
  else if (pub_yydm = '0012') or (pub_yydm = '0229') or (pub_yydm = '0240') or (pub_yydm = '0264')
    or (pub_yydm = '0263') or (pub_yydm = '0266')
  then
  begin
      ...
      begin
        ...
        begin
          ...
          begin
            if j > ( 5 - ( y * 5 - count ) ) then
            begin
              aControl := DetailBand3.FindChildControl( 'ksrqj' + IntToStr( j - 1 ) );
              if aControl is TQRLabel then
                TQRLabel( aControl ).Caption := '';
              aControl := DetailBand3.FindChildControl( 'kdysmcj' + IntToStr( j - 1 ) );
              if aControl is TQRLabel then
                TQRLabel( aControl ).Caption := '';
              DetailBand3.FindChildControl( 'qrgrimagej' + IntToStr( j ) ).enabled := false;
              //  TQRLabel( DetailBand1.FindChildControl( 'xmmc' + IntToStr( j - 1 ) ) ).Width := 0;
              TQRLabel( DetailBand3.FindChildControl( 'ypyfj' + IntToStr( j - 1 ) ) ).Caption := '';
              //TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := '';
              TQRLabel( DetailBand3.FindChildControl( 'xmmcj' + IntToStr( j - 1 ) ) ).caption := '';
            end
            else
            begin
              aControl := DetailBand3.FindChildControl( 'ksrqj' + IntToStr( j - 1 ) );
              ...
              begin
                ...
                DetailBand3.FindChildControl( 'QRgrImagej' + IntToStr( j ) ).Top := TQRLabel( DetailBand3.FindChildControl( 'xmmcj' + IntToStr( j - 1 ) ) ).top;
                DetailBand3.FindChildControl( 'QRgrImagej' + IntToStr( j ) ).left := DetailBand3.FindChildControl( 'ypyfj' + IntToStr( j - 1 ) ).left - 20;
                DetailBand3.FindChildControl( 'qrgrimagej' + IntToStr( j ) ).height := 30;
                TQRLabel( DetailBand3.FindChildControl( 'ypyfj' + IntToStr( j - 1 ) ) ).Caption := trim( sp_zxjl_cq_dy.FieldByName( 'yytj' ).asstring ) + '  ' + trim( sp_zxjl_cq_dy.FieldByName( 'ypyf' ).asstring );
                TQRLabel( DetailBand3.FindChildControl( 'xmmcj' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'xmmc' ).Asstring;
                //TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'tzrq' ).Asstring;
              end
              else
              begin
                DetailBand3.FindChildControl( 'qrgrimagej' + IntToStr( j ) ).enabled := false;
                DetailBand3.FindChildControl( 'xmmcj' + IntToStr( j - 1 ) ).Width := 235;
                TQRLabel( DetailBand3.FindChildControl( 'ypyfj' + IntToStr( j - 1 ) ) ).Caption := '';
                TQRLabel( DetailBand3.FindChildControl( 'xmmcj' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'xmmc' ).Asstring + ' ' + trim( sp_zxjl_cq_dy.FieldByName( 'yytj' ).asstring ) + ' ' + trim( sp_zxjl_cq_dy.FieldByName( 'ypyf' ).asstring );
                // TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'tzrq' ).Asstring;
              end;
              sp_zxjl_cq_dy.Next;
            end;
        ...
        QRLabel2jy0.Caption := pub_yymc + '长期输液计划执行单';
        DM_data.qry_pub.Close;
        DM_data.qry_pub.SQL.text := 'select * from sys_kscwsz where bqdm=' + '''' + pub_bqdm + '''' + ' and bcbh=' + '''' + trim( sp_jbxx.fieldbyname( 'bch' ).asstring ) + '''';
        DM_data.qry_pub.Open;
...
procedure Tfrm_yzinput_lishi.btnJCSFClick(Sender: TObject);
var
  CFpath: TStrings;
  Waycf: string;
  I: integer;
begin
  ...
  try
    ...
    ReadThirdJcJy(1, CFpath);
    if trim(CFpath.text) <> '' then
    begin
      for I := 0 to CFpath.Count - 1 do
      begin
        if Pos('Waycf',trim(CFpath.strings[I]))>0 then
          Waycf := Copy(trim(CFpath.strings[I]),Length('Waycf')+2,length(trim(CFpath.strings[I])));
      end;
    end;
  finally
    CFpath.Free;
  end;
  if (Waycf <> '') and (pub_zyh <> '') then
  begin
    Waycf := Waycf + ' ' + Trim(pub_zyh);
    //ShowMessage(Waycf);
    //ShellExecute(Handle, 'open', PChar(Waycf), nil, nil, SW_SHOWNORMAL);
    WinExec(PChar(Waycf),SW_SHOWDEFAULT);
  end;
end;

procedure Tfrm_yzinput_lishi.btnSFLISClick(Sender: TObject);
var
  CFpath: TStrings;
  Waycf, PrPatientNo, PrBillDoctor, Instr,v_IP,visitAddr: string;
  I: integer;
begin
  inherited;
//  Waycf := '';
...
//        if Pos('Waycf',trim(CFpath.strings[I]))>0 then
//          Waycf := Copy(trim(CFpath.strings[I]),Length('Waycf')+2,length(trim(CFpath.strings[I])));
//        if Pos('ZyPrPatientNo',trim(CFpath.strings[I]))>0  then
//          PrPatientNo := Copy(trim(CFpath.strings[I]),Length('ZyPrPatientNo')+2,length(trim(CFpath.strings[I])));
//        if Pos('PrBillDoctor',trim(CFpath.strings[I]))>0 then
//          PrBillDoctor := Copy(trim(CFpath.strings[I]),Length('PrBillDoctor')+2,length(trim(CFpath.strings[I])));
...
//  end;
//
//  if (PrPatientNo <> '') and (pub_zyh <> '') then
//    PrPatientNo := PrPatientNo + '='+quotedstr(Trim(pub_zyh))+'||';
//
//  if (PrBillDoctor <> '') and (pub_czyxm <> '') then
//    PrBillDoctor := PrBillDoctor + '='+quotedstr(Trim(pub_czyxm))+'||';
//
//  if PrPatientNo <> '' then
//    Instr := Instr + PrPatientNo;
//  if PrBillDoctor <> '' then
//    Instr := Instr + PrBillDoctor;
//  if Instr <> '' then Waycf := Waycf+' '+Instr;
//  if Waycf <> '' then
//  begin
//    WinExec(PChar(Waycf),SW_SHOWDEFAULT);
//  end;
  ...
  begin
    ...
    '/modules/main/rmsysMain.aspx?rmkj_userno=lis&HIS_PARAM_MODE=3';
    if trim(Edt_pass_tmh.Text) <> '' then
    begin
      visitAddr := visitAddr + '&HIS_PARAM_PATNO='+ Trim(Edt_pass_tmh.Text);
    end else
    begin
      visitAddr := visitAddr + '&HIS_PARAM_PATNO='+ Trim(pub_tmh);
    end;
  //  visitAddr := 'http://192.168.30.250/modules/main/rmsysMain.aspx?rmkj_userno=lis&HIS_PARAM_MODE=3&HIS_PARAM_PATNO=000130653&rmkj_width=1366&rmkj_height=728';
    WebBrowser1.Navigate(visitAddr);
  end;
    //ShellExecute(application.Handle, 'open', pchar(Waycf), pchar(Instr), nil, sw_show);
end;

procedure Tfrm_yzinput_lishi.btn_gtyxClick(Sender: TObject);
var
  visitAddr:string;
begin
  visitAddr := 'http://192.168.0.178/PIMSWebView.asp?ZID='+Trim(pub_zyh) +
  '&PID=病人ID' + '&EXAMID =申请单号';
  if pub_ISPACSWebXS then  //是否web浏览器显示
  begin
...
var
  z_zyh: string;
begin
  if (pub_yydm='0265') or  (pub_yydm='0136') then
  begin
    btnSFLIS.visible := true;
    FLis_IP := get_lisIP;
  end ;
  if  (pub_yydm='0136')  then
  begin
    btnJCSF.visible := False;
  end;
  btnSFLIS.visible := true;
  btnJCSF.visible := true;
  jybz:=false;
  DM_data.qry_pub.close;
  DM_data.qry_pub.SQL.text:='select * from sys_xt_kg ';
  DM_data.qry_pub.Open;
...
procedure Tfrm_yzinput_lishi.Edt_pass_tmhKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  visitAddr: string;
begin
...
begin
  ...
  begin
    ...
    begin
      ...
      //出院病人显示病人信息  170323 wangwei
      dm_data.qry_pub.close;
      dm_data.qry_pub.sql.text:='select  tmh,zyh,bah,sbkh,sbdw,brxm,pym,brxb,brsf,brjg,brmz,ryzd,dwdh,nxrdh,  '+
                               'dwyb,hkyb,csrq,brnl,brzy,brdz,zycs,zgys,mzys,fylb,bqdm,ryks,ksdm,bch,    '+
                               'dbr,dbrdz,sfzhm,rybq,rytj,ryrq,zdjf,yjze,zkze,zfze,zfy,ybndzje,ryczy,cyrq,     '+
                               'cyczy,cybz,jsbz,lcbz,byzg,qfbz,sjyjze,jsdh,jsrq,jsczy,jstjbz,djrq,sbzfje,zchzbz,  '+
                               'zchzfy,bz,gx,ylje,ba_brbm,zyts,fpdybz,dysj,fpdh,dyczy,ztjsyzyh,dybz,mzjzzh'+
                               ',datediff(day,cyrq,getdate()) cyts from zysf_cydj where zyh='+''''+zyh+''''+
                               ' union all '+
                               'select  tmh,zyh,bah,sbkh,sbdw,brxm,pym,brxb,brsf,brjg,brmz,ryzd,dwdh,nxrdh,  '+
                               'dwyb,hkyb,csrq,brnl,brzy,brdz,zycs,zgys,mzys,fylb,bqdm,ryks,ksdm,bch,    '+
                               'dbr,dbrdz,sfzhm,rybq,rytj,ryrq,zdjf,yjze,zkze,zfze,zfy,ybndzje,ryczy,cyrq,     '+
                               'cyczy,cybz,jsbz,lcbz,byzg,qfbz,sjyjze,jsdh,jsrq,jsczy,jstjbz,djrq,sbzfje,zchzbz,  '+
                               'zchzfy,bz,gx,ylje,ba_brbm,zyts,fpdybz,dysj,fpdh,dyczy,ztjsyzyh,dybz,mzjzzh,'+
                               'datediff(day,cyrq,getdate()) cyts from zysf_zydj where cybz=1 and zyh='+''''+zyh+'''';
      dm_data.qry_pub.open;
      if dm_data.qry_pub.IsEmpty then
        Exit
      else
...
begin
  ...
  DM_data.qry_pub.Close;
  DM_data.qry_pub.SQL.text := 'select a.id yzid,b.zxcs,isnull(c.zxsj,''20000101'') sj from  zybl_zyyz a, '+
  '(select yzid,count(*) zxcs,zybl_zyyz_yzzxqd_mx where yzid='+
  sp_dryz.FieldByName( 'id' ).asstring+') b,zybl_zyyz_zxjl c'+
  ' where a.id=' +
...
procedure Tfrm_yzinput_lishi.Button17Click(Sender: TObject);
type
...
begin
  ...
  begin
    ...
    if (pub_yydm = '0263') or (pub_yydm = '0264') then
      sp_dryz.Parameters.ParamByName('@vkdks').Value := pub_ksdm
    else
      sp_dryz.Parameters.ParamByName('@vkdks').Value := '';
    sp_dryz.Open;
  end
  else
