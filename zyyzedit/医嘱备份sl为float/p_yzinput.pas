...
uses
  ...
  cxButtons, OleCtrls, SHDocVw, ImgList, ToolWin,
  cxGridBandedTableView, dateutils, cxCheckBox, EMRPAD30Lib_TLB, p_obj,
  OleServer,IniFiles,PassInterfaceClass,
...
type
  TTmyrect = array[0..300] of Tmyrect;   //100

  type
//合理用药动态调用定义
  _MDC_Quit = function(): Integer; stdcall;
  _MDC_Init = function(const pcCheckMode: pchar; const pcHiscode: pchar; const pcDoctorcode: pchar): integer; stdcall;
  _MDC_SetPatient = function(const pcpatcode: pchar; //病人ID
  ...
  _MDC_AddMedCond = function(const pcIndex: pchar;
    const pcDiseaseCode: pchar;
    const pcDiseaseName: pchar;
    const pcRecipNo: pchar
    ): integer; stdcall;
  _MDC_AddScreenDrug = function(const pcIndex: pchar;
  ...
  _MDC_DoSetDrug = function(const druguniquecode: pchar;
    const drugname: pchar
    ): integer; stdcall;
  _MDC_DoRefDrug = function(piQueryType: integer): integer; stdcall;
  _MDC_CloseDrugHint = function(): integer; stdcall;
  _MDC_DoCheck = function(const piShowMode: integer;
    const piIsSave: integer
    ): integer; stdcall;
  _MDC_GetWarningCode = function(const pcIndex: pchar): integer; stdcall;
  _TIHandle = Integer;
type
  Tfrm_yzinput = class(TfrmAdoReconBase)
    ...
    procedure lblSBRJKXXClick(Sender: TObject);
    procedure lblXJKBRXXClick(Sender: TObject);    //procedure qrbnd3AfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
  private
    ...
    function JkxxShow(Inflag: Integer): string;
  protected
    procedure showForm;
  public
    tmh, zyh, brxm, lszyh, xzljyzid,zy_sfzhm: string; //住院号
...
var
  ...
  FMDC_Quit: _MDC_Quit;
  FMDC_Init: _MDC_Init;
  FMDC_SetPatient: _MDC_SetPatient;
  FMDC_AddMedCond: _MDC_AddMedCond;
  FMDC_AddScreenDrug: _MDC_AddScreenDrug;
  FMDC_DoSetDrug: _MDC_DoSetDrug;
  FMDC_DoRefDrug: _MDC_DoRefDrug;
  FMDC_CloseDrugHint: _MDC_CloseDrugHint;
  FMDC_DoCheck: _MDC_DoCheck;
  FMDC_GetWarningCode: _MDC_GetWarningCode;
  Fhmq: _TIHandle;
...
uses p_dm, p_func, p_yzzt, p_treeview_ctxz, p_bylsyz, p_tjyz,
  ...
  p_zysf_print_service, p_main, p_lcljyzxzdr, p_zyjs, p_dzjcsqd,p_PassDllClare, //p_cqyzxd,
  p_lsyzxd, p_cqyzxd;
...
begin
  ...
  if sp_dryz.RecordCount < 1500 then //1000
    createrect(myrect_cx, sp_dryz, dbgrideh4, 10, 11, 16);  //16
    ...
    begin
     ...
     // qry_yz.Edit;
      ...
      begin
        ...
        qry_yz.FieldByName('sl').AsFloat := sp_cx_yz_xmzhmx.FieldByName('fysl').AsFloat;
...
begin
  //查看pacs设置
  DM_data.qry_pub.close;
  DM_data.qry_pub.SQL.text := 'select value from sys_xt_xkg   where name ='+QuotedStr('mzyz_pacs_lb');
  try
   DM_data.qry_pub.Open;
  except
  end;
  FPACS := DM_data.qry_pub.FieldByName('value').asinteger;
  showForm;
  //------2011-11-23-jc 合理用药初始化------------
  if pub_pass_version = '3' then //老版合理用药 3.0
  begin
    try
      if LoadPassDll then
      begin
  ...
  //新版合理用药初始化
    ...
    if (FileExists('PASS4Invoke.dll')) and (gi_passenabled = 1) then
    begin
      Fhmq := LoadLibrary('PASS4Invoke.dll');
      @FMDC_Quit:= GetProcAddress(Fhmq, 'MDC_Quit');
      @FMDC_Init := GetProcAddress(Fhmq, 'MDC_Init');
      @FMDC_SetPatient:= GetProcAddress(Fhmq, 'MDC_SetPatient');
      @FMDC_AddMedCond := GetProcAddress(Fhmq, 'MDC_AddMedCond');
      @FMDC_AddScreenDrug := GetProcAddress(Fhmq, 'MDC_AddScreenDrug');
      @FMDC_DoSetDrug := GetProcAddress(Fhmq, 'MDC_DoSetDrug');
      @FMDC_DoRefDrug := GetProcAddress(Fhmq, 'MDC_DoRefDrug');
      @FMDC_CloseDrugHint := GetProcAddress(Fhmq, 'MDC_CloseDrugHint');
      @FMDC_DoCheck := GetProcAddress(Fhmq, 'MDC_DoCheck');
      @FMDC_GetWarningCode := GetProcAddress(Fhmq, 'MDC_GetWarningCode');
      FMDC_Quit();
      if (FMDC_Init(pchar('zy'), pchar(pub_yymc), pchar(pub_czydm)) = 1) then
        gi_passenabled := 1
         else
        gi_passenabled := 0;
    end else
...
begin
  ...
  begin
    ...
    sp_jbxx.Parameters.ParamValues['@zyh'] := pub_zyh;
    sp_jbxx.Open;
...
begin
  ...
  begin
    ...
    if (gi_passenabled = 1)then
    begin
      if  (grid_yzlr.SelectedIndex = 2)  then
      begin
        if FMDC_DoSetDrug(PChar(trim(qry_yz.FieldByName('xmdm').AsString)), PChar(trim(qry_yz.FieldByName('xmmc').AsString))) <> 1 then
        begin
         exit;
        end;
        FMDC_CloseDrugHint();
        ...
        if FMDC_DoRefDrug(51) <> 1 then
        begin
          exit;
        end;
      end else
      begin
        if FMDC_DoSetDrug(PChar(''), PChar('')) <> 1 then
        begin
          exit;
        end;
        FMDC_CloseDrugHint();
...
//      end;
  ...
  if ((qry_yz.FieldByName('lb').AsString = '检验') or
   (qry_yz.FieldByName('lb').AsString = '检查') or
   (qry_yz.FieldByName('lb').AsString = '治疗'))  and (pub_yydm='0266')then
  begin
    qry_yz.Edit;
  ...
  //新版合理用药-yansghuai2016-03-02新加----------------------------------    if pub_pass_version = '3' then //老版合理用药

  if (gi_passenabled = 1) and ( pub_pass_version = '4') then   //新版合理用药4.0
  begin
    if  (grid_yzlr.SelectedIndex = 2)  then
    begin
      if FMDC_DoSetDrug(PChar(trim(qry_yz.FieldByName('xmdm').AsString)), PChar(trim(qry_yz.FieldByName('xmmc').AsString))) <> 1 then
...
begin
  if pub_pass_version = '3' then //如果是老版本合理用药3.0
  begin
...
//  end;
  ...
  res := IntToStr(FMDC_SetPatient(PChar(tmh),
    PChar(zyh),
    '1',
...
begin
  ...
  qry_icd10.Filter :=  'JBMC=' + #39 + DBEdit10.text + #39;
 // qry_icd10.Filter := 'JBMC = ''+ DBEdit10.text + ''' ;
...
//    l_icdname := md.firstname;
    ...
    begin
      if FMDC_AddMedCond(PChar(IntToStr(i)), PChar(md.firstcode), PChar(md.firstname), '') <> 1 then
      begin
        Result := False;
        exit;
      end else
  ...
  try
    ...
    begin
      ...
      begin
        ...
        l_sldw := trim(qry_yz.FieldByName('dw').AsString);
        l_bz := trim(qry_yz.FieldByName('bz').AsString);
        if not ((l_yzlb = '药品') or (l_yzlb = '成药') or (l_yzlb = '中药')) then
          break;
        if FMDC_AddScreenDrug(PChar(IntToStr(pass_i)), pass_i, PChar(l_xmdm), PChar(l_xmmc), PChar(l_ypyl),
          PChar(l_yldw), PChar(l_ypyf), PChar(l_yytj), PChar(l_yytj), PChar(FormatDateTime('yyyy-MM-dd hh:mm:ss',frm_tjyz.cxDateEdit1.Value)),
          PChar(FormatDateTime('yyyy-MM-dd hh:mm:ss',frm_tjyz.cxDateEdit2.Value)),PChar(frm_func.func_getServerDate), PChar(l_fzph), PChar(vyzsx),'0',PChar(pub_ksdm),PChar(pub_ksmc),
...
{-------------------------------------------------------------------------------
  过程名:    Tfrm_yzinput.wf_recipe_check_pass4
  作者:      Administrator
  日期:      2016.03.08
  参数:      aitem: integer; var qry_pass: TADOQuery; qry_yf: TADOQuery; amount: integer
  返回值:    integer
-------------------------------------------------------------------------------}
function Tfrm_yzinput.wf_recipe_check_pass4(aitem: integer; var qry_pass: TADOQuery; qry_yf: TADOQuery; amount: integer): integer;
...
begin
  ...
  begin
    ...
    piShowMode := 1;
    piIsSave := 1;
    FMDC_DoCheck(piShowMode, piIsSave);
    FMDC_DoSetDrug('', '');
    FMDC_CloseDrugHint();
  //调用审查函数
//    for I := 1 to amount do
//    begin
//      iwarn := FMDC_GetWarningCode(PChar(IntToStr(i)));
//      if iwarn = 1 then
//      begin
//       Break;
//      end;
...
//    end;
    iwarn := FMDC_GetWarningCode(PChar('')); //传入空，自动获取pass最高级别
    result := iwarn;// imaxwarn; //返回最严重警示级别
  end;
end;

{-------------------------------------------------------------------------------
  过程名:    Tfrm_yzinput.proc_save_pass4
  作者:      Administrator
  日期:      2016.03.07
  参数:      aitem: Integer; var qry_pass: TADOQuery; qry_yf: TADOQuery; amount: Integer
  返回值:    Boolean
-------------------------------------------------------------------------------}
function Tfrm_yzinput.proc_save_pass4(aitem: Integer; var qry_pass: TADOQuery; qry_yf: TADOQuery; amount: Integer): Boolean;
var
...
begin
  ...
  begin
    ...
    if pub_pass_ygkzbz and (li_warn = 1) then
    begin
...
type
  TIntFunc = procedure(tmh,addr,rid:string);stdcall;
 var
  Th:Thandle;
  Tf:TIntFunc;
  Tp:TFarProc;
...
begin
  ...
  else if FPACS = 2  then   //(pub_yydm = '0012') or (pub_yydm='0229') then //三台中医院
  begin
...
//    end;
    ...
    sp_pacs_jg.Open;
    if sp_pacs_jg.recordcount=0 then
    begin
      application.messagebox('还没有出检查结果,请稍后重试！','提示',mb_ok);
      exit;
    end;
    ...
    WebBrowser1.Navigate(visitAddr);
    if sp_pacs_jg.FieldByName('zt').AsString='dll' then
      begin
        if not FileExists( ExtractFileDir(Application.ExeName) + '\pacs_xy.dll' ) then
        begin
          application.MessageBox('更新缺少文件,请稍后重试！','提示',mb_ok);
          UpdateMe( ExtractFileDir(Application.ExeName) + '\pacs_xy.dll' );
          exit;
        end;
        th := LoadLibrary( 'pacs_xy.dll' );
        if Th>0 then
        begin
          try
            Tp:=GetProcAddress(Th,PChar('Pacsreprot'));
            if Tp<>nil then
            begin
              Tf:=TIntFunc(Tp);
              Tf(tmh,sp_pacs_jg.FieldByName('addr').AsString,id); {调用函数}
            end
            else
            ShowMessage('没有找到对应函数入口。');
          finally
            FreeLibrary(Th); {释放DLL}
          end
        end
        else
        ShowMessage('缺少文件pacs_xy.dll。');
      end
    else
    begin
...
begin
  ...
  if (pub_pass_version = '3') and (gi_passenabled = 1) then //老版合理用药3.0
  begin
    PassQuit; //退出
  end else if  (pub_pass_version = '4') and (gi_passenabled = 1) then //新版合理用药4.0
  begin
    FMDC_Quit;
  end;
end;
...
begin
  ...
  begin
        if sp_dryz.RecordCount < 1500 then   //800
        begin
          createrect(myrect_cx, sp_dryz, dbgrideh4, 10, 11, 16); //16
        end;
  end;
...
//  end;
  ...
  try
    ...
    frm_tjyz.tmh := tmh;
    frm_tjyz.zyh := zyh;
    frm_tjyz.F_passEnabled := gi_passenabled; //合理用赋值
...
begin
 ...
 // TDBGridEh(dbgrideh4).selectedindex:=TDBGridEh(dbgrideh4).selectedindex+1 ;
...
begin
  ...
  else if (pub_yydm = '0012') or (pub_yydm = '0229') or (pub_yydm = '0240') then
  begin
...
{-------------------------------------------------------------------------------
  过程名:    Tfrm_yzinput.JkxxShow
  作者:      yangshuai
  日期:      2016.06.08
  参数:      Inflag: Integer
  返回值:    string
-------------------------------------------------------------------------------}
function Tfrm_yzinput.JkxxShow(Inflag: Integer): string;
var
Myfile : TIniFile;
Url,Uid,verifycode,Outvalue,doctorNo,Filename : string;
begin
  Result := '';
  Url :='';
  Uid :='';
  ...
  doctorNo :='';
  Filename :='';
  Filename := ExtractFilePath(Paramstr(0))+'JKXXPZ.INI';
  ...
  begin
    ...
    0:
      begin
        Url :=Trim(Myfile.ReadString('SJKXX','Url',''));
        Uid :=Trim(Myfile.ReadString('SJKXX','Uid',''));
        if Uid <>'' then Uid :='uid='+Uid;
        Outvalue := Url + Uid;
      end;
    1:
      begin
        Url :=Trim(Myfile.ReadString('XJKXX','Url',''));
        Uid :=Trim(Myfile.ReadString('XJKXX','Uid',''));
        verifycode :=Trim(Myfile.ReadString('XJKXX','verifycode',''));
        doctorNo :=Trim(Myfile.ReadString('XJKXX','doctorNo',''));
        if Uid <>'' then Uid :='duns='+Uid;
        if verifycode<>'' then verifycode := '&verifycode='+verifycode;
        if doctorNo<>'' then  doctorNo := '&doctorNo='+ doctorNo;
        Outvalue := Url + Uid + verifycode + doctorNo;
      end;
    end;
    Myfile.Free;
  end
  ...
  begin
    Myfile.WriteString('SJKXX','Url','');
    Myfile.WriteString('SJKXX','Uid','');
    Myfile.WriteString('XJKXX','Url','');
    Myfile.WriteString('XJKXX','Uid','');
    Myfile.WriteString('XJKXX','verifycode','');
    Myfile.WriteString('XJKXX','doctorNo','');
    Myfile.Free;
    Application.MessageBox('请在JKXXPZ.INI文件中设置健康信息的连接地址及其它内容!',PChar('温馨提示'),MB_OK);
    Exit;
  end;
  Result := Outvalue;
end;

procedure Tfrm_yzinput.lblSBRJKXXClick(Sender: TObject);
var
INurl : string;
begin
  INurl := JkxxShow(0);
  if trim(sp_jbxx.fieldbyname('sfzhm').asstring)<>'' then INurl := INurl+'&sfzh='+trim(sp_jbxx.fieldbyname('sfzhm').asstring);
  //shellexcute 根据连接地址在当前位置打开IE浏览器
  if INurl <>'' then ShellExecute(Application.Handle, nil,PChar(INurl), nil, nil, SW_SHOW);
  //OpenInIe(INurl);
end;
procedure Tfrm_yzinput.lblXJKBRXXClick(Sender: TObject);
var
  AINurl : string;
begin
  AINurl := JkxxShow(1);
  if trim(sp_jbxx.fieldbyname('sfzhm').asstring)<>'' then  AINurl := AINurl + '&archiveId='+trim(sp_jbxx.fieldbyname('sfzhm').asstring);
  //shellexcute 根据连接地址在当前位置打开IE浏览器
  if AINurl <>'' then ShellExecute(Application.Handle, nil,PChar(AINurl), nil, nil, SW_SHOW);
  //OpenInIe(AINurl)
end;
