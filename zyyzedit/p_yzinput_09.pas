...
uses
  ...
  cxGridBandedTableView,dateutils;
type
  Tmyrect = record
    ph: string;
...
uses p_dm, p_func, p_yzzt, p_treeview_ctxz, p_treeview_ctwh, p_bylsyz, p_tjyz, p_yzfz,
  ...
  p_bqgl_sssq,p_LCLJSJ,p_ljzxxz,p_lcljzx, p_yszx_yzinput,p_xz_jhddy;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if ( qry_yz.FieldByName( 'lb' ).asstring = '7' ) or ( qry_yz.FieldByName( 'lb' ).asstring = 'y1' ) or ( qry_yz.FieldByName( 'lb' ).asstring = '药一' ) then
                begin
                  if pub_yydm='0015' then
...
var zyzd,icd_10,ljid,sSql,dqrq,ljrq,jdid,dqjdid,mrrq:string;
    Frm_LCLJSJ: TFrm_LCLJSJ;
    vBBH,ljts,zxts,sczt,qnl,znl:Integer;
    zxrq,ljxzjg,_yzsj,_yzsx,_id,_zhzxsj,_fzph,_brxb,_brnl1,_brnl2,_brnl3,_ryts,_synl:string;
    yztzsj:TDateTime;
begin
  byyy:='';
  ljxzjg:='3';
...
//    DM_data.qry_pub.Open;
  ...
  begin
    ...
    begin
      ...
      if not DM_data.qry_pub.IsEmpty then
      begin
        Application.MessageBox('路径出现执行出现变异情况，请选择操作', '警告',
    ...
    try
      ...
      qry_ljzxmx.open;
      qry_ljxmjc.close;
      qry_ljxmjc.sql.text:='select * from LCLJ_LCLJXM where ljid='+''''+ljid+''''+' and jdid='+
      ''''+jdid+''''+' and bbh='+inttostr(vbbh);
      qry_ljxmjc.open;
      qry_ljxmjc.First;
...
begin
  ...
  begin
    ...
    begin
      application.MessageBox( '停止日期不能在开单日期之前！', '提示' );
      exit;
    end;
    if _fzph <> '' then
    begin
  ...
  begin
    if (grid_yzlr.SelectedIndex = 2) and
      ...
      try
        ...
        DM_data.qry_pub.FieldByName('blid').Value:=frm_yszx_yzinput.edit1.Text;
        if frm_yszx_yzinput.edt_sfjj.itemindex = 1 then begin//紧急
...
begin
  ...
  //  grid_yzlr.width:=scrollbox4.width-1;
   // Application.OnMessage := frm_yzinput.OnMouseWheel_yzzx;
end;
