...
uses
  ...
  cxGridBandedTableView,dateutils, cxCheckBox, EMRPAD30Lib_TLB,p_obj, OleServer,
  grproLib_TLB;
type
  Tmyrect = record
    ph: string;
...
type
  Tfrm_yzinput = class( TForm )
    ...
    GridppReport10: TGridppReport;
    sp_jcsqd: TADOStoredProc;
    ...
    //procedure qrbnd3AfterPrint(Sender: TQRCustomBand; BandPrinted: Boolean);
  private
    jybz:Boolean;
    ktqsj,bl_bcid,blbcpdid,bc_count:integer;
    v_fylbmc,locatename:string;
    pub_blys,pub_zzys,pub_zgys,pub_ssys,pub_jfys,pub_hzys,bl_zgys,addbcjl:string;
    pub_blysbz,pub_zzysbz,pub_zgysbz,bl_blysbz,bl_zzysbz,bl_zgysbz:Boolean;
  ...
  public
    ...
    ls_zylsj: single;
    scbz, ztid: integer;
    clph, xmph, ypph, jcph, jyph, qtph,bl_bllb: string; //材料，治疗，药品，检查，检验,其它
...
const
  ...
  jysqddm = '021902'; //检验申请单
  jcsqddm = '021901'; //检查申请单
implementation
...
uses p_dm, p_func, p_yzzt, p_treeview_ctxz, p_treeview_ctwh, p_bylsyz, p_tjyz, p_yzfz,
  ...
  p_bqgl_sssq,p_LCLJSJ,p_ljzxxz,p_lcljzx, p_yszx_yzinput,p_xz_jhddy,p_yxsq,
  p_zysf_print_service,p_main,p_lcljyzxzdr;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        //---------------药品用药途径-----------------------------
        if ( trim( qry_yz.FieldByName( 'lb' ).AsString ) = '药品' ) or ( trim( qry_yz.FieldByName( 'lb' ).AsString ) = '成药' ) or
           ( trim( qry_yz.FieldByName( 'lb' ).AsString ) = '药一' ) or
           ( trim( qry_yz.FieldByName( 'lb' ).AsString ) = '草药' ) then
        begin
        ...
        if ( ( pub_yydm = '0013' ) and ( qry_yz.FieldByName( 'xmdm' ).asstring = '01000001000357' )
          ...
          ( ( pub_yydm = '0013' ) and ( qry_yz.FieldByName( 'lb' ).asstring = '治疗' ) and
        ...
        begin
          ...
          if (qry_xmyz.FieldByName( 'lb' ).AsString <> qry_yz.FieldByName( 'lb' ).AsString) and (qry_yz.FieldByName( 'lb' ).AsString<>'药一') then
          begin
...
begin
  ...
  begin
    ...
    begin
      DM_data.qry_pub.close;
      DM_data.qry_pub.sql.text:='select a.ypdm,round(a.zxdwsl/zyyfzhl,2) sl from xyf_ypkc a,sys_ypdm b where a.ypdm='+''''+
      qry_yz.FieldByName( 'xmdm' ).asstring+''''+' and a.ypdm=b.ypdm and a.ksdm='+''''+
      sp_cx_yp_yz.Parameters.ParamByName('@ksdm').value+'''';
      DM_data.qry_pub.open;

      if qry_yz.FieldByName( 'sl' ).AsInteger >DM_data.qry_pub.FieldByName( 'sl' ).asfloat  then
      begin
        application.MessageBox( '输入数量大于库存，请重新输入！', '提示', 0 + 48 );
        qry_yz.FieldByName( 'sl' ).AsInteger := 1;
        grid_yzlr.SelectedIndex := 7;
      end;
    end;
    ...
    if ( qry_yz.fieldbyname( 'yzsx' ).asstring = '临时' ) and (( qry_yz.fieldbyname( 'lb' ).asstring = '药品' ) or ( qry_yz.fieldbyname( 'lb' ).asstring = '药一' ))
      and ( Trim( qry_yz.fieldbyname( 'ypyf' ).asstring ) = '' ) then
    begin
      with sp_cx_yzyljssl do
      begin
    ...
    if ( ( yzlb = '治疗' ) or ( yzlb = '检验' ) or ( yzlb = '检查' ) ) and ( grid_yzlr.SelectedField.FieldName = 'ypyf' ) then
    begin
      DM_data.qry_pub.Close;
...
begin
  ...
  if lcljms then begin
    DBGridEh7.left := 2 - ScrollBox4.HorzScrollBar.Position;
  end else begin
    DBGridEh7.left := 36 - ScrollBox4.HorzScrollBar.Position;
  end;
end;
...
begin
  ...
  qry_yz.Filter := ' (lb = ' + '''' + '药品' + '''' + ' and yzsx <> ' + #39 + '长期' + #39+') or '+
  ...
  qry_yz.Filtered := false;
  qry_yz.Filter := ' lb <> ' + '''' + '材料' + '''' + ' and lb <> ' + '''' + '药品' + '''' + ' and lb<>''药一'' and yzsx <> ' + #39 + '长期' + #39;
  qry_yz.Filtered := true;
  if qry_yz.RecordCount > 0 then
  begin
    qry_yzlist.Insert;
...
begin
  qry_yz.Edit;
  if (trim( qry_yz.FieldByName( 'lb' ).AsString ) = '药品') or (trim( qry_yz.FieldByName( 'lb' ).AsString ) = '药一') then
  begin
    {    if ypph = '' then
        begin
...
begin
  ...
  if qry_yz.RecordCount = 0 then
    exit;
  dm_data.qry_pub.Close;
  dm_data.qry_pub.SQL.Text := 'select * from lclj_blmbdx where ztmc=' + #39 + zyyz_mbmc + #39 + ' and ' +
    'czy=' + #39 + pub_czydm + #39;
  dm_data.qry_pub.open;
  if not dm_data.qry_pub.isempty then
  begin
  ...
  dm_data.qry_pub.Close;
  dm_data.qry_pub.SQL.Text := 'insert into lclj_blmbdx(bqdm,ksdm,ysdm,ztmc,czy,mzbz,zybz,mblb,bqmc,ksmc) values(' +
    #39 + pub_bqdm + #39 + ',' + #39 + pub_ksdm + #39 + ',' + #39 + pub_czydm + #39 +
    ',' + #39 + zyyz_mbmc + #39 + ',' + #39 + pub_czydm + #39 + ',' + inttostr(0) + ',' + inttostr(0) + ',' +
    #39 + zyyz_mblb + #39 + ',' + #39 + pub_bqmc + #39 + ',' + #39 + pub_ksmc + #39 + ')';
  dm_data.qry_pub.ExecSQL;
  dm_data.qry_pub.Close;
  dm_data.qry_pub.SQL.Text := 'select ztid from lclj_blmbdx where ztmc=' + #39 + zyyz_mbmc + #39 +
...
begin
  ...
  if pub_yydm='0015' then
  begin
    if (Pos('农合病人',v_fylbmc)>0) and  (sp_cx_yp_yz.FieldByName('nhlb').AsBoolean=false) then
    begin
      if sbnhbryykz=true then
      begin
        Application.MessageBox('农合病人不能非农合用药！', '提错误示', MB_OK + MB_ICONSTOP);
        Exit;
      end
      else
      begin
    ...
    begin
      if sbnhbryykz=true then
      begin
        Application.MessageBox('社保病人不能开社保自费药！', '提错误示', MB_OK + MB_ICONSTOP);
        Exit;
      end
      else
      begin
...
begin
  ...
  while not qry_yz.Eof do
  begin
    if (( qry_yz.FieldByName( 'lb' ).AsString = '药品' )) or ( qry_yz.FieldByName( 'lb' ).AsString = '药一' ) then
      if ypid = '' then
        //获取药品ID列表
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if ( qry_yz.FieldByName( 'lb' ).asstring = '7' ) or ( qry_yz.FieldByName( 'lb' ).asstring = 'cy' ) or ( qry_yz.FieldByName( 'lb' ).asstring = '草药' ) then
              begin
                qry_yz.FieldByName( 'lb' ).value := '草药';
              end
              else
                if ( qry_yz.FieldByName( 'lb' ).asstring = '8' ) or ( qry_yz.FieldByName( 'lb' ).asstring = 'y1' ) or ( qry_yz.FieldByName( 'lb' ).asstring = '药一' ) then
                begin
                  if pub_yydm='0015' then
...
begin
  ...
  dm_data.qry_pub.close;
  dm_data.qry_pub.sql.text:='select a.*,b.mc fylbmc from zysf_zydj a,sys_fylb b where a.zyh='+''''+zyh+''''+' and a.fylb=b.dm';
  dm_data.qry_pub.Open;
  ...
  sp_dryz.Open;
  DM_data.qry_pub.close;
  DM_data.qry_pub.sql.text:='select * from lclj_bricd10 a,icd10 b where a.icd_10=b.jbbm and zyh='+''''+zyh+'''';
  DM_data.qry_pub.Open;
  if not DM_data.qry_pub.IsEmpty then
  begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                TQRLabel( qrbndcqsy.FindChildControl( 'cqsyypyf' + IntToStr( j - 1 ) ) ).Caption := trim( sp_zxjl_cq_dy.FieldByName( 'yytj' ).asstring ) + '  ' + trim( sp_zxjl_cq_dy.FieldByName( 'ypyf' ).asstring );
                TQRLabel( qrbndcqsy.FindChildControl( 'cqsyxmmc' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'xmmc' ).Asstring;
                //TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'tzrq' ).Asstring;
              end
              else
              begin
                ...
                TQRLabel( qrbndcqsy.FindChildControl( 'cqsyypyf' + IntToStr( j - 1 ) ) ).Caption := '';
                TQRLabel( qrbndcqsy.FindChildControl( 'cqsyxmmc' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'xmmc' ).Asstring + ' ' + trim( sp_zxjl_cq_dy.FieldByName( 'yytj' ).asstring ) + ' ' + trim( sp_zxjl_cq_dy.FieldByName( 'ypyf' ).asstring );
                // TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'tzrq' ).Asstring;
              end;
              sp_zxjl_cq_dy.Next;
            end;
...
begin
  ...
  begin
    ...
    prv_cyfdm := DM_data.qry_pub.FieldByName( 'cyyf' ).AsString;
...
begin
  ...
  try
    ...
    qry_lxd.Close;
    qry_lxd.SQL.Text:='select a.*,c.*,b.BLOOD_TYPE_NAME  ,case Rh when ''1'' then ''阴'' else'+
...
begin
  ...
  v_yytj:='';
  v_lb :=   '(lb=' + '''' + '草药' + '''' + ' and  tzrq=null and kdks=' + '''' + pub_ksdm + ''')' + ' or ';
...
var zyzd,icd_10,ljid,sSql,dqrq,ljrq,jdid,dqjdid,mrrq,ljdrsj,mbdm,zgysdm:string;
    Frm_LCLJSJ: TFrm_LCLJSJ;
    vBBH,ljts,zxts,sczt,qnl,znl:Integer;
    zxrq,ljxzjg,_yzsj,_yzsx,_id,_zhzxsj,_fzph,_brxb,_brnl1,_brnl2,_brnl3,_ryts,_synl,zhrq:string;
    yztzsj:TDateTime;
begin
  DM_data.qry_pub.close;
  DM_data.qry_pub.SQL.text:='select * from zysf_zydj where  zyh='+''''+zyh+'''';
  ...
  DM_data.qry_pub.close;
  DM_data.qry_pub.SQL.Text:='select max(rq) rq from lclj_brljzx_mx where  zyh='+''''+zyh+'''';
  DM_data.qry_pub.Open;
  ...
  begin
    ...
    begin
      DM_data.qry_pub.close;
      DM_data.qry_pub.SQL.Text:='select * from sys_xt_kg where rtrim(isnull(lcljdrsj,''''))<>''''';
      DM_data.qry_pub.Open;
      if not DM_data.qry_pub.IsEmpty then
      begin
        ...
        begin
          Application.MessageBox(pchar('在当天'+ljdrsj+'之前不能提前导入第二天路径！'),
          '提示', MB_OK + MB_ICONWARNING);
          Exit;
        end;
...
//    DM_data.qry_pub.Open;
    ...
    if icd_10='' then
    begin
      application.MessageBox( '未设置病人ICD10病种，不能导入相应路径！', '提示', 0 + 48 );
      Exit;
    end;
    DM_data.qry_pub.close;
    DM_data.qry_pub.SQL.Text:='select * from LCLJ_LCLJBZ  where jbid='+''''+icd_10+'''';
    DM_data.qry_pub.Open;
    if DM_data.qry_pub.IsEmpty then
    begin
      application.MessageBox( '此病种没有对应的临床路径，不能导入路径！', '提示', 0 + 48 );
      Exit;
    end;
    ljid:=DM_data.qry_pub.FieldByName('ljid').AsString;
    DM_data.qry_pub.close;
    DM_data.qry_pub.SQL.Text:='select * from LCLJ_LCLJML  where ljid='+''''+ljid+'''';
    DM_data.qry_pub.Open;
    if (DM_data.qry_pub.FieldByName('syxb').AsString='1') and (_brxb='女') then
    begin
      application.MessageBox( '性别不匹配，不能导入路径！', '提示', 0 + 48 );
      exit;
    end
    else if (DM_data.qry_pub.FieldByName('syxb').AsString='2') and (_brxb='男') then
    begin
      application.MessageBox( '性别不匹配，不能导入路径！', '提示', 0 + 48 );
      exit;
    end;
    ...
    begin
      ...
      begin
        znl:=strtoint(copy(_synl,pos('-',_synl)+1,pos('岁',_synl)-pos('-',_synl)-1));
      ...
      begin
        znl:=strtoint(copy(_synl,pos('-',_synl)+1,pos('月',_synl)-pos('-',_synl)-1));
      ...
      begin
        znl:=strtoint(copy(_synl,pos('-',_synl)+1,pos('日',_synl)-pos('-',_synl)-1));
    ...
    begin
      ...
      begin
        application.MessageBox( '超过了导入限定天数，不能导入路径！', '提示', 0 + 48 );
        exit;
      end;
    end;
  end;
  ...
  DM_data.qry_pub.close;
  DM_data.qry_pub.SQL.Text:='select * from LCLJ_LCLJKS  where ljid='+''''+ljid+'''';
  DM_data.qry_pub.Open;
  if (DM_data.qry_pub.IsEmpty) or (DM_data.qry_pub.FieldByName('ksid').AsString=pub_ksdm) then
  begin
    cxGrid1.Visible:=True;
    cxGrid1.Align:=alClient;
    sSql := 'Select * from LCLJ_LCLJBB where  ISNULL(SHSJ,0)<>0 and LJID=' + #39 + ljid + #39 ;
    DM_data.qry_pub.Close;
    ...
    mrrq:=FormatDateTime('yyyyMMdd',Frm_func.curr_date+1);
    DM_data.qry_pub.Close;
    DM_data.qry_pub.SQL.text:= 'select max(convert(datetime,rq)) rq,count(*) ts from lclj_brljzx where zyh='+''''+zyh+'''';
    DM_data.qry_pub.open;
    if Trim(DM_data.qry_pub.FieldByName('rq').AsString)='' then
    begin
    ...
    begin
      ljrq:= FormatDateTime('yyyyMMdd',DM_data.qry_pub.FieldByName('rq').AsDateTime+1);
      ljts:=DM_data.qry_pub.FieldByName('ts').AsInteger+1;
      zxrq:= FormatDateTime('yyyyMMdd',DM_data.qry_pub.FieldByName('rq').AsDateTime);
      zxts:=DM_data.qry_pub.FieldByName('ts').AsInteger;
      //开始判断头一天路径执行情况
      DM_data.qry_pub.Close;
      DM_data.qry_pub.SQL.text:= 'select * from lclj_lcljjd where ljid='+''''+ljid+''''+
      ' and ksts<='+inttostr(zxts)+' and case jsts when 0 then ksts else jsts end >='+inttostr(zxts);
      DM_data.qry_pub.open;
      dqjdid:=DM_data.qry_pub.FieldByName('jdid').AsString;
      DM_data.qry_pub.close;
      DM_data.qry_pub.SQL.text:='select * from LCLJ_LCLJXM where ljid='+''''+ljid+''''+
      ' and jdid='+''''+dqjdid+''''+' and zxfs=1 and xmid not in (select xmid from lclj_brljzx_mx where zyh='
      +''''+zyh+''''+ ' and '+' rq='+''''+ zxrq+''' and zxjg=''1'') ';
      DM_data.qry_pub.open;
      if not DM_data.qry_pub.IsEmpty then
      begin
        ...
        while not DM_data.qry_pub.eof do
        begin
           byyy:= byyy+DM_data.qry_pub.FieldByName('xmnr').asstring+'【未执行】'+#13+#10;
           DM_data.qry_pub.Next;
        end;
        DM_data.qry_pub.close;
        DM_data.qry_pub.SQL.text:='select * from LCLJ_LCLJXM where ljid='+''''+ljid+''''+
        ' and jdid='+''''+dqjdid+''''+' and zxfs=4 and xmid in (select xmid from lclj_brljzx_mx where zyh='
        +''''+zyh+''''+ ' and zxjg=''1'' group by xmid having count(*)>1) ';
        DM_data.qry_pub.open;
        DM_data.qry_pub.First;
        while not DM_data.qry_pub.eof do
        begin
        ...
        DM_data.qry_pub.close;
        DM_data.qry_pub.SQL.text:='select * from lclj_yzby where rq='+''''+zxrq+''''+' and zyh='
        +''''+zyh+'''';
        DM_data.qry_pub.open;
        DM_data.qry_pub.First;
        while not DM_data.qry_pub.eof do
        begin
           ...
           begin
             yzbyyy:= yzbyyy+'新增医嘱：'+DM_data.qry_pub.FieldByName('yznr').asstring+#13+#10;
           end;
           if DM_data.qry_pub.FieldByName('bz').asstring='未用医嘱' then
           begin
             yzbyyy:= yzbyyy+'以下这条医嘱未用：'+DM_data.qry_pub.FieldByName('yyznr').asstring+#13+#10;
           end;
           if DM_data.qry_pub.FieldByName('bz').asstring='修改医嘱' then
           begin
             yzbyyy:= yzbyyy+'从原医嘱：'+DM_data.qry_pub.FieldByName('yyznr').asstring+'  修改为现医嘱：'+
    ...
    begin
      DM_data.qry_pub.close;
      DM_data.qry_pub.SQL.text:='delete from lclj_brljby where zyh='+''''+zyh+''''+
      ' and rq='+''''+zxrq+'''';
      DM_data.qry_pub.ExecSQL;
      DM_data.qry_pub.close;
      DM_data.qry_pub.SQL.text:='insert into lclj_brljby select '+''''+zyh+''''+
      ','+''''+zxrq+''''+','+''''+byyy+'医嘱变异：'+yzbyyy+''''+','+'''变异退出''';
      DM_data.qry_pub.ExecSQL;
      DM_data.qry_pub.close;
      DM_data.qry_pub.SQL.text:='update lclj_brlclj set zt=0 where zyh='+''''+zyh+'''';
      DM_data.qry_pub.ExecSQL;
      DM_data.qry_pub.close;
      DM_data.qry_pub.SQL.text:='insert into lclj_brljtc select '+''''+zyh+''''+
      ','+''''+zxrq+''''+','+''''+byyy+'医嘱变异：'+yzbyyy+''''+','+'''路径中退出''';
      DM_data.qry_pub.ExecSQL;
      Exit;
    end
    else if ljxzjg='1' then
    begin
      if Application.MessageBox( '选择变异停止会停止所有长期医嘱，是否继续?', '询问', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON1 ) = IDYES then
      begin
        DM_data.qry_pub.close;
        DM_data.qry_pub.SQL.text:='delete from lclj_brljby where zyh='+''''+zyh+''''+
        ' and rq='+''''+zxrq+'''';
    ...
    DM_data.qry_pub.Close;
    DM_data.qry_pub.SQL.text:= 'select * from lclj_lcljjd where ljid='+''''+ljid+''''+
    ' and ksts<='+inttostr(ljts)+' and case jsts when 0 then ksts else jsts end >='+inttostr(ljts);
    DM_data.qry_pub.open;
    if DM_data.qry_pub.IsEmpty then
    begin
      application.MessageBox( '住院天数已超出路径天数，路径结束！', '提示', 0 + 48 );
      exit;
    end
    else
    begin
    ...
    try
      ...
      begin
        DM_data.qry_pub.close;
        DM_data.qry_pub.sql.Text:='insert into LCLJ_brlclj select '+''''+ljid+''''+','+
        ''''+zyh+''''+',1,'+inttostr(vbbh);
        DM_data.qry_pub.ExecSQL;
        refresh_yz;
      end;
      ...
      begin
        qry_lcljmbjc.Close;
        qry_lcljmbjc.SQL.text:='select * from lclj_blmbdx where ztmc='+''''+ljid+''''+' and '+
        ...
        begin
          ...
          sp_lcljdr.Parameters.ParamByName('@xmid').value:=qry_ljxmjc.fieldbyname('xmid').asstring;
          ...
          lcljkdrq:=Copy(ljrq,1,4)+'-'+Copy(ljrq,5,2)+'-'+Copy(ljrq,7,2)+' 00:00:00';
          refresh_yz;
          saveyz_tj;
          //saveyz_lcljtj;
        end;
        ...
        ljid+''''+' and jdid='+''''+jdid+''' and rtrim(isnull(mbdm,0))<>0';
        qry_dzblmbcx.Open;
      ...
      DM_data.qry_pub.Close;
      DM_data.qry_pub.sql.text:='delete from lclj_brljzx where zyh='+''''+zyh+''''+' and '+
      ' rq='+''''+ljrq+'''';
      DM_data.qry_pub.ExecSQL;
      DM_data.qry_pub.Close;
      DM_data.qry_pub.sql.text:='delete from lclj_brljzx_mx where zyh='+''''+zyh+''''+' and '+
      ' rq='+''''+ljrq+'''';
      DM_data.qry_pub.ExecSQL;
      DM_data.qry_pub.Close;
      DM_data.qry_pub.sql.text:='delete from lclj_yzby where zyh='+''''+zyh+''''+' and '+
      ' rq='+''''+ljrq+'''';
      DM_data.qry_pub.ExecSQL;
      DM_data.qry_pub.Close;
      DM_data.qry_pub.sql.text:='select * from lclj_brljzx where zyh='+''''+zyh+'''';
      DM_data.qry_pub.open;
      if DM_data.qry_pub.IsEmpty then
      begin
        DM_data.qry_pub.Close;
        DM_data.qry_pub.sql.text:='delete from LCLJ_brlclj where zyh='+''''+zyh+''''+' and '+
        ' zt=1 ';
        DM_data.qry_pub.ExecSQL;
      end;
    ...
    Frm_LCLJSJ := TFrm_LCLJSJ.Create(Application);
    Frm_LCLJSJ.ShowMXData_rq(ljid, vBBH,frm_yzinput.cxGrid1BandedTableView1,mrrq,zyh,frm_yzinput);
  end
  else
  begin
    application.MessageBox( '没有适合本科室使用的路径，不能导入路径！', '提示', 0 + 48 );
    Exit;
  end;
end;
...
begin
  ...
  begin
    DM_data.qry_pub.close;
    DM_data.qry_pub.SQL.text:='select left(rtrim(dm),2) mblb,mbmc from sys_dzbl_mbmx where qybz=1 and zybz=1 and  mbdm='+mbdm;
    DM_data.qry_pub.Open;
    if DM_data.qry_pub.IsEmpty then
    Exit;
    ...
    yzblys(mblb); //验证病人是不是其它科室转入,病历医生是否已做调整,并取出手术医生,及监护医生
    ...
    begin
      ...
      begin
        qry_brbl.Close;
        qry_brbl.SQL.Text := 'select * from zydzbl_zydzblmx where fldm=' + '''' + mblb + '''' +
          ' and zyh=' + '''' + zyh + '''';
        qry_brbl.Open;
    ...
    begin
      ...
      try
        ...
        stream := TADOBlobStream.Create(DM_data.qry_pub.FieldByName('mbwj') as TBlobField, bmRead);
        stream.Position := 0;
      ...
      begin
        ...
        blbcpdid := frm_main.EMRPad301.GetBaseLineCount; //缓回当前文档的行数记录病程片段ID
    ...
    begin
      if (bl_bllb = '02') then
        application.MessageBox(pchar('你是：[' + trim(brxm) + ']本科执班医生,您只能在病程类别中加入病程!'), '提示', 0 + 48)
      else if bl_bllb = ',03,07' then
        application.MessageBox(pchar('你是：[' + trim(brxm) + ']的麻醉医生,您只能在手术,知情同意书中类别中加入病历!'), '提示', 0 + 48)
      else if bl_bllb = '04' then
        application.MessageBox(pchar('你是：[' + trim(brxm) + ']的会诊医生,您只能在会诊病历类别中加入病历!'), '提示', 0 + 48)
      else if bl_bllb = ',05,06' then
        application.MessageBox(pchar('你是：[' + trim(brxm) + ']的ICU医生,您只能在护理病历类别中加入病历!'), '提示', 0 + 48)
      else
        application.MessageBox(pchar('你不是：[' + trim(brxm) + ']的病历医生!'), '提示', 0 + 48);
    end;
  end
  else
...
//  end;
    ...
    begin
      DM_data.qry_pub.Close;
      DM_data.qry_pub.SQL.Text := 'select max(bcid) maxid from zydzbl_zybcjlmx where zyh=' + '''' + zyh + '''';
      DM_data.qry_pub.Open;
      ...
      frm_main.EMRPad301.universalboolfunction('', '', 47, bl_bcid); //设置病历片段号
    end;
    linecount := frm_main.EMRPad301.GetBaseLineCount;
    frm_main.EMRPad301.SetSel(0, -1, 0, 0, 0, linecount - 1, -1, linecount - 1, 0, 0); //选中全部文档
    ...
    begin
      ...
      DM_data.qry_pub.Close;
      DM_data.qry_pub.SQL.Text := 'select max(blid) maxid from zydzbl_zydzblmx where zyh=' + '''' + zyh + '''';
      DM_data.qry_pub.Open;
      if DM_data.qry_pub.FieldByName('maxid').AsString = '' then
        pub_blid := '001'
      else
        pub_blid := formatex(strtoint(DM_data.qry_pub.FieldByName('maxid').AsString) + 1, 3);
      if (mblb = '02') then
...
begin
  ...
  sp_up_bcjlmx.Parameters.ParamByName('@blbcmc').Value := pub_filename + '.' + inttostr(bl_bcid);
...
begin
  ...
  DM_data.qry_pub.Close;
  DM_data.qry_pub.SQL.Text := 'select * from zydzbl_zydzblmx where fldm=' + '''' + mblb + '''' +
    ' and zyh=' + '''' + zyh + '''';
  DM_data.qry_pub.Open;
  try
    stream := TADOBlobStream.Create(DM_data.qry_pub.FieldByName('blwj') as TBlobField, bmRead);
    stream.Position := 0;
    stream1 := TFileStream.Create(filename, fmCreate);
    stream1.Position := 0;
    stream1.CopyFrom(stream, stream.Size);
...
begin
  ...
  begin
    ...
    if DM_data.Qry_pub.IsEmpty then
    begin
      Application.MessageBox('没有设置医生授权!请先进入病历医生进行医生授权!', '提示', 0 + 48);
      exit;
    end;
...
begin
  ...
  DM_data.qry_pub.close;
  DM_data.qry_pub.SQL.Text := 'select * from sys_czy where dm in (' + #39 + zzysdm + #39 + ',' + #39 + zgysdm + #39 +
    ',' + #39 + blysdm + #39 + ',' + #39 + pub_czydm + #39 + ',' + #39 + bl_mzys + #39 + ')';
  DM_data.qry_pub.Open;
  if not ((pos(pub_czydm, bl_mzys) > 0) or (pos(pub_czydm, bl_ssys) > 0) or (pos(pub_czydm, bl_jfys) > 0)
          or (pos(pub_czydm, bl_hzys) > 0)) then //当前医生不是病人麻醉,手术、icu医生,会诊医生
  begin
  ...
  ksdm := trim(DM_data.qry_pub.FieldByName('ksdm').AsString); //操作员当前科室代码
  DM_data.qry_pub.Close;
  DM_data.qry_pub.SQL.Text := 'select * from sys_bqzyfl where bqzydm=' + #39 + ksdm + #39;
  DM_data.qry_pub.Open;
  bqdm := trim(DM_data.qry_pub.FieldByName('bqdm').AsString);
  bl_bllb := '';
  if (pub_czydm = zzysdm) or (pub_czydm=blysdm ) or (pub_czyxm=bl_blys) or (pub_czyxm=bl_zzys) then
  else
  begin
    if (mblb = '03') and (pub_yydm='0198') then
    begin
    ...
    begin
      if ((pub_bqdm = bqdm) and (pub_czydm <> bl_mzys)) and ((mblb = '02') or (mblb = '11')) then
      begin
        bl_bllb := ',02,11';
      end
      else if ((pos(pub_czydm, bl_ssys) > 0) or (pub_czydm = bl_mzys)) and ((mblb = '03') or (mblb = '07') )  then //是病人手术医生或是麻醉医生，使用03类别下模板
        bl_bllb := ',03,07,02'
      else if (pos(pub_czydm, bl_jfys) > 0) then//and  ((copy(pub_key, 1, 2) = '05') or (copy(pub_key, 1, 2) = '06') ) then
        bl_bllb := ',05,06,02'
...
begin
  ...
  //
  for I := 0 to zz.PageCount - 1 do begin
...
begin
  ...
  jybz:=False;
  if lcljms then HideSomething ; //临床路径模板
  ...
  prv_cyfdm := Trim( DM_data.qry_pub.FieldByName( 'cyyf' ).AsString );
  ...
  if Trim(sp_jbxx.FieldByName( 'zgys' ).AsString)<>pub_czydm then
  begin
...
begin
  if key = 13 then begin
    if passAddr='' then exit;
    
    visitAddr:='http://'+passAddr+'/webviewer/query.do?inHospitalNo='+inttostr(strtoint(Edt_pass_tmh.Text));
...
begin
  ...
  begin
    if ( sp_dryz.FieldByName( 'lb' ).asstring = '药品' ) or ( sp_dryz.FieldByName( 'lb' ).asstring = '成药' ) or
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                TQRLabel( qrbndcqzs.FindChildControl( 'cqzsypyf' + IntToStr( j - 1 ) ) ).Caption := trim( sp_zxjl_cq_dy.FieldByName( 'yytj' ).asstring ) + '  ' + trim( sp_zxjl_cq_dy.FieldByName( 'ypyf' ).asstring );
                TQRLabel( qrbndcqzs.FindChildControl( 'cqzsxmmc' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'xmmc' ).Asstring;
                //TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'tzrq' ).Asstring;
              end
              else
              begin
                ...
                TQRLabel( qrbndcqzs.FindChildControl( 'cqzsypyf' + IntToStr( j - 1 ) ) ).Caption := '';
                TQRLabel( qrbndcqzs.FindChildControl( 'cqzsxmmc' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'xmmc' ).Asstring + ' ' + trim( sp_zxjl_cq_dy.FieldByName( 'yytj' ).asstring ) + ' ' + trim( sp_zxjl_cq_dy.FieldByName( 'ypyf' ).asstring );
                // TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'tzrq' ).Asstring;
              end;
              sp_zxjl_cq_dy.Next;
            end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                TQRLabel( qrbndcqzl.FindChildControl( 'cqzlypyf' + IntToStr( j - 1 ) ) ).Caption := trim( sp_zxjl_cq_dy.FieldByName( 'yytj' ).asstring ) + '  ' + trim( sp_zxjl_cq_dy.FieldByName( 'ypyf' ).asstring );
                TQRLabel( qrbndcqzl.FindChildControl( 'cqzlxmmc' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'xmmc' ).Asstring;
                //TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'tzrq' ).Asstring;
              end
              else
              begin
                ...
                TQRLabel( qrbndcqzl.FindChildControl( 'cqzlypyf' + IntToStr( j - 1 ) ) ).Caption := '';
                TQRLabel( qrbndcqzl.FindChildControl( 'cqzlxmmc' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'xmmc' ).Asstring + ' ' + trim( sp_zxjl_cq_dy.FieldByName( 'yytj' ).asstring ) + ' ' + trim( sp_zxjl_cq_dy.FieldByName( 'ypyf' ).asstring );
                // TQRLabel( DetailBand1.FindChildControl( 'tzrq' + IntToStr( j - 1 ) ) ).caption := sp_zxjl_cq_dy.FieldByName( 'tzrq' ).Asstring;
              end;
              sp_zxjl_cq_dy.Next;
            end;
...
begin
  ...
  begin
    ...
    while not DM_data.qry_pub.eof do
    begin
      v_yytj := v_yytj + '(yytj=' + '''' + DM_data.qry_pub.FieldByName( 'yytj' ).asstring + '''' + ' and lb<>''草药'' and  tzrq=null and kdks=' + '''' + pub_ksdm + ''')' + ' or ';
      DM_data.qry_pub.Next;
    end;
...
begin
  ...
  begin
    ...
    if FormatDateTime('yyyy-MM-dd hh:mm:ss',strtodatetime( tzyzsj ))<=
       FormatDateTime('yyyy-MM-dd hh:mm:ss',sp_dryz.FieldByName( 'kdrq' ).asdatetime) then
    begin
      application.MessageBox( '停止日期不能在开单日期之前或等于开单日期！', '提示' );
      exit;
    end;
    if _fzph <> '' then
    begin
...
begin
  ...
  begin
    ...
    while not DM_data.qry_pub.eof do
    begin
      v_yytj := v_yytj + '(yytj=' + '''' + DM_data.qry_pub.FieldByName( 'yytj' ).asstring + '''' + ' and  xzbz=1 and tzrq=null and kdks=' + '''' + pub_ksdm + ''')' + ' or ';
      DM_data.qry_pub.Next;
    end;
  ...
  begin
    ...
    while not DM_data.qry_pub.eof do
    begin
      v_lb := v_lb + '(lb=' + '''' + DM_data.qry_pub.FieldByName( 'lbmc' ).asstring + '''' + ' and  tzrq=null and xzbz=1 and kdks=' + '''' + pub_ksdm + ''')' + ' or ';
      DM_data.qry_pub.Next;
    end;
...
begin
  ...
  begin
    ...
    while not DM_data.qry_pub.eof do
    begin
      v_yytj := v_yytj + '(yytj=' + '''' + DM_data.qry_pub.FieldByName( 'yytj' ).asstring + '''' + ' and  tzrq=null and xzbz=1 and kdks=' + '''' + pub_ksdm +
        '''' + ' and kdrq>=' + '''' + formatdatetime( 'yy-MM-dd', frm_func.curr_date ) + ' 00:00' + ''')' + ' or ';
      DM_data.qry_pub.Next;
    end;
  ...
  begin
    ...
    while not DM_data.qry_pub.eof do
    begin
      v_lb := v_lb + '(lb=' + '''' + DM_data.qry_pub.FieldByName( 'lbmc' ).asstring + '''' + ' and  xzbz=1 and tzrq=null and kdks=' + '''' + pub_ksdm +
        '''' + ' and kdrq>=' + '''' + formatdatetime( 'yy-MM-dd', frm_func.curr_date ) + ' 00:00' + ''')' + ' or ';
      DM_data.qry_pub.Next;
    end;
  ...
  begin
    filter := v_yytj + v_lb + ' ( xmmc like ''%皮试%'' and xzbz=1 '+ ' and kdrq>=' + ''''
    + formatdatetime( 'yy-MM-dd', frm_func.curr_date ) + ' 00:00'')';
  end;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  qru_yzxg.close;
                  qru_yzxg.sql.text := 'update zybl_zyyz set kdks=' + #39 + yz_kdks + #39 + ',zxbz = 0,xgrq = ' +
...
begin
  ...
  v_yytj:='';
  v_lb :=   '(lb=' + '''' + '草药' + '''' + ' and  tzrq=null and kdks=' + '''' + pub_ksdm +''''
  + ' and kdrq>=' + '''' + formatdatetime( 'yy-MM-dd', frm_func.curr_date ) + ' 00:00' + ''')'  + ' or ';
...
begin
  ...
  if ( gi_PassEnabled = 0 ) or ( PassGetState( '0' ) = 0 ) or
    ...
    Exit;
  PassDoCommand( 402 );
  if ( grid_yzlr.SelectedIndex = 2 ) and ( trim( qry_yz.FieldByName( 'xmmc' ).AsString ) <> '' ) then
...
//    qry_yz.Post;
  ...
  begin
    ...
    if ( (pos('zyysgzzt03',lowercase(pub_czyqx)) <> 0) or (pub_czydm = '9999')) and
       ( yzlb = '其它' ) and
       ( (Pos( '术', Trim( grid_yzlr.Columns[2].DisplayText ) ) <> 0) or
         (False)  //zksx='手术'
       )and
       (grid_yzlr.SelectedIndex = 2)then begin
      case 
        Application.MessageBox('医嘱中出现“术”字，是否添加择期手术安排申请？'
        + #13#10#13#10 + '点击  “是”：进入择期手术安排程序' + #13#10 + 
        '        “否”：为急诊手术，按原方式进行', '提示', MB_YESNO + MB_ICONQUESTION) of
        IDYES:
          begin
            try
...
begin
  s:=recno;
  if (grid_yzlr.SelectedIndex=15) and (qry_yz.fieldbyname('lb').asstring='草药')
...
begin
  if ( qry_yz.FieldByName( 'lb' ).AsString = '药品' ) or  ( qry_yz.FieldByName( 'lb' ).AsString = '药一' ) or ( ( qry_yz.FieldByName( 'lb' ).AsString = '材料' ) and isbqkf ) then
  begin
    yzNew( fs );
  end
  else
...
begin
  ...
  grid_yzlr.SelectedIndex := 7;
  if ( qry_yz.FieldByName( 'lb' ).AsString = '药品' ) or ( trim( qry_yz.FieldByName( 'lb' ).AsString ) = '成药' ) or
     ( qry_yz.FieldByName( 'lb' ).AsString = '药一' ) or ( qry_yz.FieldByName( 'lb' ).AsString = '草药' ) then
  begin
    try
...
begin
  DM_data.qry_pub.close;
  DM_data.qry_pub.SQL.text:='select * from zysf_zydj where cybz=0 and jsbz=0 and zyh='+''''+zyh+'''';
  DM_data.qry_pub.Open;
  if DM_data.qry_pub.IsEmpty then
  begin
...
begin
  ...
  begin
    ...
    if ( qry_yz.FieldByName( 'lb' ).AsString = '草药' ) and ( Trim( qry_yz.FieldByName( 'fzph' ).AsString ) = '' )
...
begin
  DM_data.qry_pub.close;
  dm_data.qry_pub.sql.text:='select * from lclj_bricd10 where zyh='+''''+zyh+'''';
  dm_data.qry_pub.open;
  if dm_data.qry_pub.IsEmpty then
  begin
...
//  DM_data.qry_pub.ExecSQL;
  ...
  begin
    DM_data.qry_pub.Close;
    DM_data.qry_pub.SQL.text:='insert into BLOOD_test values('+#39+qry_lxd.FieldByName('APPLY_NUM').AsString+#39+','+
    #39+zyh+#39+','+'1,0)';
    DM_data.qry_pub.ExecSQL;
  end
  else
  begin
    DM_data.qry_pub.Close;
    DM_data.qry_pub.SQL.text:='update BLOOD_test set smallint=1 where apply_num='+#39+qry_lxd.FieldByName('APPLY_NUM').AsString+#39;
    DM_data.qry_pub.ExecSQL;
  end;
              //重新绑定刷新
  qry_lxd.Close;
...
try
  ...
  begin
    ...
    DM_data.qry_pub.Close;
    DM_data.qry_pub.SQL.text:= 'select max(rq) rq,count(*) ts from lclj_brljzx where zyh='+''''+zyh+'''';
    DM_data.qry_pub.open;
    ...
    DM_data.qry_pub.close;
    DM_data.qry_pub.SQL.text:='insert into lclj_brljtc select '+''''+zyh+''''+
    ','+''''+tcrq+''''+','+''''+'退出原因：'+trim(edit4.text)+''''+','+'''直接退出''';
    DM_data.qry_pub.ExecSQL;
...
procedure Tfrm_yzinput.Button15Click(Sender: TObject);
var jysqd:TCusBb;
    res:string;
    yzid:string;
begin
  ...
  sp_jysqddy.Parameters.ParamByName('@kssqrq').value:=DateTimePicker3.DateTime;
  ...
  begin
    jysqd := TCusBb.Create;
    jysqd.GetBbByDM( jysqd,jysqddm );
    GridppReport10.LoadFromStr( jysqd.nr );
    GridppReport10.DataSet := nil;
    GridppReport10.DataSet := sp_jysqddy;
    if CheckBox1.Checked=True then
    begin
       GridppReport10.ParameterByName( '是否急查' ).Value := '急查';
    end
    else
    begin
      GridppReport10.ParameterByName( '是否急查' ).Value := '';
    end;
    res := frm_func.SetCReportXtcs( GridppReport10 );
    if res <> '1' then
    begin
      ShowMessage( res );
    end
    ...
    begin
      GridppReport10.PrintPreview(false);
    end;
  end;
  sp_cx_drjyxm.close;
  sp_cx_drjyxm.Parameters.Refresh;
  sp_cx_drjyxm.Parameters.ParamByName('@zyh').Value:=zyh;
  sp_cx_drjyxm.Parameters.ParamByName('@kssqrq').Value:=DateTimePicker3.DateTime;
  sp_cx_drjyxm.Parameters.ParamByName('@bqdm').Value:=pub_bqdm;
  sp_cx_drjyxm.Parameters.ParamByName('@czydm').Value:=pub_czydm;
  sp_cx_drjyxm.Open;
end;

procedure Tfrm_yzinput.Button16Click(Sender: TObject);
var jcsqd:TCusBb;
    res:string;
begin
  sp_jcsqd.Close;
  sp_jcsqd.Parameters.Refresh;
  sp_jcsqd.Parameters.ParamByName('@zyh').value:=zyh;
  sp_jcsqd.Parameters.ParamByName('@kdrq').value:=DateTimePicker3.DateTime;
  sp_jcsqd.Parameters.ParamByName('@bqdm').value:=pub_bqdm;
  sp_jcsqd.Open;
  if not sp_jcsqd.isempty then
  begin
    jcsqd := TCusBb.Create;
    jcsqd.GetBbByDM( jcsqd,jcsqddm );
    GridppReport11.LoadFromStr( jcsqd.nr );
    GridppReport11.DataSet := nil;
    GridppReport11.DataSet := sp_jcsqd;
    res := frm_func.SetCReportXtcs( GridppReport11 );
    if res <> '1' then
    begin
      ShowMessage( res );
    end
    ...
    begin
      GridppReport11.PrintPreview(false);
    end;
  end;

end;

procedure Tfrm_yzinput.Button19Click(Sender: TObject);
begin
  sp_cx_drjyxm.close;
  sp_cx_drjyxm.Parameters.Refresh;
  sp_cx_drjyxm.Parameters.ParamByName('@zyh').Value:=zyh;
...
begin
  ...
  begin
    DM_data.qry_pub.Close;
    DM_data.qry_pub.SQL.text:='select c.* from zyxyf_zycfzb a,zysf_zyfymx b,zyxyf_zycfmx c'+
    ...
    begin
      ...
      begin
        ...
        qry_yz.FieldByName( 'lb' ).Value := '其它';
        qry_yz.FieldByName( 'xmmc' ).Value := trim( DM_data.qry_pub.FieldByName( 'ypmc' ).asstring );
        qry_yz.FieldByName( 'sl' ).Value :=  DM_data.qry_pub.FieldByName( 'ypsl' ).AsFloat ;
        qry_yz.FieldByName( 'czks' ).Value := pub_zyxyfdm;
        qry_yz.FieldByName( 'kdks' ).Value := yz_kdks;
        qry_yz.FieldByName( 'zyh' ).Value := zyh;
        qry_yz.FieldByName( 'tmh' ).Value := tmh;
        qry_yz.FieldByName( 'xmdm' ).Value := trim( DM_data.qry_pub.FieldByName( 'ypdm' ).asstring );
        qry_yz.FieldByName( 'dw' ).Value := trim( DM_data.qry_pub.FieldByName( 'ypdw' ).asstring );
        qry_yz.FieldByName( 'ypdwid' ).Value := DM_data.qry_pub.FieldByName( 'ypdwid' ).AsInteger;
        qry_yz.FieldByName( 'ypbzbl' ).Value := DM_data.qry_pub.FieldByName( 'ypbzbl' ).AsFloat;
        qry_yz.FieldByName( 'yplsj' ).Value := DM_data.qry_pub.FieldByName( 'ypdj' ).AsFloat;
        qry_yz.FieldByName( 'ypcgj' ).Value := DM_data.qry_pub.FieldByName( 'kccgj' ).AsFloat;
        qry_yz.FieldByName( 'ypyl' ).Value := DM_data.qry_pub.FieldByName( 'yl' ).AsFloat;
        qry_yz.FieldByName( 'yldw' ).Value := DM_data.qry_pub.FieldByName( 'yldw' ).asstring;
        qry_yz.FieldByName( 'yytj' ).Value := DM_data.qry_pub.FieldByName( 'yytj' ).asstring;
        qry_yz.FieldByName( 'ypyf' ).Value := DM_data.qry_pub.FieldByName( 'yf' ).asstring;
        qry_yz.FieldByName( 'yplb' ).Value := '';
        qry_yz.FieldByName( 'gg' ).Value := '';
...
begin
  Application.OnMessage := nil;
  DM_data.qry_pub.close;
  DM_data.qry_pub.SQL.text:='select * from lclj_brljzx_mx where zyh='+''''+zyh+'''';
  DM_data.qry_pub.Open;
  if DM_data.qry_pub.IsEmpty then
  begin
...
begin
  ...
  try
    ...
    begin
      ...
      MedicomPass := TMedicomPass.Create( pub_czydm, pub_czyxm, pub_ksdm, pub_ksmc, true );
    end
    else
      gi_passenabled := 0;
  except
...
begin
  ...
  begin
    if sp_dryz.RecordCount<500 then
    begin
      createrect( myrect_cx, sp_dryz, dbgrideh4, 10, 11, 16 );
    end;
  end;
...
begin
  ...
  begin
    if (sP_dryz.fieldbyname( 'kdrq' ).AsDateTime<sP_dryz.fieldbyname( 'xgrq' ).AsDateTime) and
      (hoursbetween(sP_dryz.fieldbyname( 'kdrq' ).AsDateTime,sP_dryz.fieldbyname( 'xgrq' ).AsDateTime)>ktqsj) then
    begin
...
//          li_Warn := qry_yz.FieldByName( 'pass_warn' ).AsInteger; //'[当前行审查结果警告值]'
          ...
          //药品名称、给药途径名称、警告值信息。
          PassSetQueryDrug( PChar( ls_DrugCode ), PChar( ls_DrugName ), PChar( ls_Unit ), PChar( ls_RouteName ) );
...
begin
  ...
  if DM_data.qry_pub.RecordCount > 0 then
  begin
    if application.MessageBox( '当前有医嘱未执行，是否还要开转入医嘱?', '提示信息', mb_iconquestion + mb_okcancel + mb_defbutton1 ) = idcancel then
      abort;
  end
  else
  begin
    if pub_yydm = '0015' then
    begin
      if application.MessageBox( '当前所有医嘱都已执行，是否要开转入医嘱?', '提示信息', mb_iconquestion + mb_okcancel + mb_defbutton1 ) = idcancel then
        abort;
    end;
  end;

  if Application.MessageBox( '添加转入医嘱，会停止原所有长期医嘱。下一步选择停止医嘱时间，' + '是否确认添加?', '询问', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON1 ) = IDYES then
  begin
    yztzsj := p_pub_sj.func_sjxz( );
    if yztzsj = strtodatetime( '1900-01-01' ) then
      Exit
...
begin
  ...
  if DM_data.qry_pub.RecordCount > 0 then
  begin
    if application.MessageBox( '当前有医嘱未执行，是否还要重整医嘱?', '提示信息', mb_iconquestion + mb_okcancel + mb_defbutton1 ) = idcancel then
      abort;
  end
  else
  begin
    if pub_yydm = '0015' then
    begin
      if application.MessageBox( '当前所有医嘱都已执行，是否要重整医嘱?', '提示信息', mb_iconquestion + mb_okcancel + mb_defbutton1 ) = idcancel then
        abort;
    end;
  end;

  if Application.MessageBox( '重整医嘱，会停止原所有长期医嘱。下一步选择停止医嘱时间，' + '是否确认添加?', '询问', MB_YESNO + MB_ICONQUESTION + MB_DEFBUTTON1 ) = IDYES then
  begin
    yztzsj := p_pub_sj.func_sjxz( );
    if yztzsj = strtodatetime( '1900-01-01' ) then
      Exit
...
begin
  ...
  begin
    ...
    if (DM_data.qry_pub.FieldByName( 'lb' ).AsString = '药品') or (DM_data.qry_pub.FieldByName( 'lb' ).AsString = '药一')  then
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if DM_data.qry_pub.FieldByName( 'sl' ).asfloat > sp_ty_ypcx.FieldByName( 'ypsyzxsl' ).asfloat then
            begin
              sftf := '0';
            end;
        ...
        begin
          ...
          begin
            ...
            if DM_data.qry_pub.FieldByName( 'sl' ).asfloat > qry_cx.FieldByName( 'sl' ).asfloat then
            begin
              sftf := '0';
            end;
        end;
