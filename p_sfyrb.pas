...
uses
  ...
  ExtCtrls, Qrctrls, DBCtrls, GridsEh, Menus, cxLookAndFeelPainters, cxButtons;

type
  Tfrm_sfyrb = class(TForm)
    Panel1: TPanel;
    Label1: TLabel;
    ...
    procedure ad_sfrbCalcFields(DataSet: TDataSet);
...
var
  frm_sfyrb: Tfrm_sfyrb;
  lb: integer;

implementation
uses p_dm, p_func, p_login, p_czyrb_all;
{$R *.DFM}

procedure Tfrm_sfyrb.BitBtn1Click(Sender: TObject);
begin
  ad_sfrb.close;
...
procedure Tfrm_sfyrb.FormShow(Sender: TObject);
begin
  dtp1.date := date - 1;
  dtp2.date := date - 1;
...
procedure Tfrm_sfyrb.QRDBText2Print(sender: TObject; var Value: string);
begin
  if (value = '0.00') or (value = '0') then
    value := '';
end;

procedure Tfrm_sfyrb.BitBtn2Click(Sender: TObject);
begin
...
procedure Tfrm_sfyrb.BitBtn3Click(Sender: TObject);
begin
  Close;
end;

procedure Tfrm_sfyrb.BitBtn4Click(Sender: TObject);
begin
  application.CreateForm(Tfrm_czyrb_all, frm_czyrb_all);
  try
    frm_czyrb_all.dt1.DateTime := dtp1.DateTime;
    frm_czyrb_all.ShowModal;
  finally
    frm_czyrb_all.free;
  end;
end;

procedure Tfrm_sfyrb.syjzePrint(sender: TObject; var Value: string);
begin
  if (value = '0.00') or (value = '0') then
    value := '';
end;

procedure Tfrm_sfyrb.dtp1Change(Sender: TObject);
begin
  ad_sfrb.close;
end;

procedure Tfrm_sfyrb.BitBtn5Click(Sender: TObject);
begin
...
procedure Tfrm_sfyrb.DBGridEh3DrawColumnCell(Sender: TObject;
...
procedure Tfrm_sfyrb.QRDBText24Print(sender: TObject; var Value: string);
begin
  if sp_czyyjmx.fieldbyname('hzbz').asboolean = TRUE then
    value := ' 待结帐';
end;

procedure Tfrm_sfyrb.DBGridEh1DblClick(Sender: TObject);
begin
  sp_czyyjmx.Close;
  if lb = 1 then
...
procedure Tfrm_sfyrb.DBGridEh1KeyPress(Sender: TObject; var Key: Char);
begin
  if key = #13 then
  begin
    sp_czyyjmx.Close;
...
procedure Tfrm_sfyrb.ad_sfrbCalcFields(DataSet: TDataSet);
begin
{ad_sfrb['yjssje']:=ad_sfrb.fieldbyname('syjze').asfloat;
ad_sfrb['jsssje']:=ad_sfrb.fieldbyname('cybxje').asfloat-ad_sfrb.fieldbyname('cytxje').asfloat+
                  ad_sfrb.fieldbyname('qffy').asfloat-ad_sfrb.fieldbyname('zchzfy').asfloat;
ad_sfrb['hjje']:=ad_sfrb.fieldbyname('syjze').asfloat+
                ad_sfrb.fieldbyname('cybxje').asfloat-ad_sfrb.fieldbyname('cytxje').asfloat+
                ad_sfrb.fieldbyname('qffy').asfloat-ad_sfrb.fieldbyname('zchzfy').asfloat;}
end;
initialization
  RegisterClass(Tfrm_sfyrb);
end.
